...
void ReleaseUnusedArrowMemory() { arrow::default_memory_pool()->ReleaseUnused(); }
...
class CrtReaderBenchmark : public FormatBenchFixtureBase<false, false> {
  public:
  void TearDown(::benchmark::State& st) override { FormatBenchFixtureBase<false, false>::TearDown(st); }
  ...
  arrow::Result<ConcurrentReadMetrics> RunConcurrentChunkReaders(const PreparedData& prepared,
                                                                 const api::Properties& properties,
                                                                 size_t reader_count,
      ...
      workers.emplace_back([&, reader_id]() {
        ...
        for (size_t i = 0; i < chunks_per_reader; ++i) {
          ...
          if (batch) {
            ...
            bytes_read.fetch_add(CalculateRecordBatchRawDataSize(batch), std::memory_order_relaxed);
          }
        }
      });
    }
  ...
  arrow::Result<ConcurrentReadMetrics> RunConcurrentGetAllChunksReaders(const PreparedData& prepared,
                                                                        const api::Properties& properties,
  ...
  void ReportCommon(::benchmark::State& st,
    ...
    st.counters["chunks"] =
        ::benchmark::Counter(static_cast<double>(prepared.chunk_count), ::benchmark::Counter::kDefaults);
  ...
  void RunGetAllChunksCase(::benchmark::State& st,
    ...
    for (auto _ : st) {
      BENCH_ASSERT_AND_ASSIGN(
          auto metrics,
          RunConcurrentGetAllChunksReaders(prepared, properties, chunk_indices, reader_count, warmup_reads), st);
    ...
    ReportCommon(st, prepared, config, format, mode, reader_count, warmup_reads, total_open_wall_ms,
                 total_warmup_wall_ms, total_read_wall_ms, total_wall_ms, peak_threads, total_rows_read,
                 total_bytes_read, label_suffix);
  }
};
...
BENCHMARK_DEFINE_F(CrtReaderBenchmark, ConcurrentOpenRead)(::benchmark::State& st) {
  ...
  for (auto _ : st) {
    BENCH_ASSERT_AND_ASSIGN(
        auto metrics, RunConcurrentChunkReaders(prepared, properties, reader_count, chunks_per_reader, warmup_reads),
        st);
    st.SetIterationTime(metrics.read_wall_ms / 1000.0);
    total_open_wall_ms += metrics.open_wall_ms;
    total_warmup_wall_ms += metrics.warmup_wall_ms;
  ...
  ReportCommon(st, prepared, config, format, mode, reader_count, warmup_reads, total_open_wall_ms, total_warmup_wall_ms,
               total_read_wall_ms, total_wall_ms, peak_threads, total_rows_read, total_bytes_read,
...
BENCHMARK_DEFINE_F(CrtReaderBenchmark, ConcurrentOpenTake)(::benchmark::State& st) {
  ...
  auto config = VectorConfig(
      kTakeNumRows, false, "vector/" + std::to_string(kTakeNumRows) + "rows/" + std::to_string(kTakeVectorDim) + "dim",
      "crt_reader_take");
  BENCH_ASSERT_STATUS_OK(PrepareData(format, properties, config, VectorProjection(), &prepared), st);
  ...
  for (auto _ : st) {
    BENCH_ASSERT_AND_ASSIGN(
        auto metrics, RunConcurrentTakeReaders(prepared, properties, reader_count, rows_per_take, warmup_reads), st);
    st.SetIterationTime(metrics.read_wall_ms / 1000.0);
    total_open_wall_ms += metrics.open_wall_ms;
    total_warmup_wall_ms += metrics.warmup_wall_ms;
    total_read_wall_ms += metrics.read_wall_ms;
...
BENCHMARK_DEFINE_F(CrtReaderBenchmark, ConcurrentOpenGetAllChunks)(::benchmark::State& st) {
  ...
  const auto mode = static_cast<CrtIoMode>(st.range(1));
  const auto reader_count = static_cast<size_t>(st.range(2));
  RunGetAllChunksCase(st, format, mode, reader_count,
                      VectorConfig(kGetAllChunksNumRows, true,
                                   "vector/" + std::to_string(kGetAllChunksRawBytes / (1024 * 1024)) + "MiB_random",
                                   "crt_reader_get_all_chunks"),
                      kGetAllChunksRawBytes, "get_all_chunks/64MiB_random_vector");
}
...
BENCHMARK_DEFINE_F(CrtReaderBenchmark, ConcurrentOpenGetAllChunks2GiB)(::benchmark::State& st) {
  ...
  const auto reader_count = static_cast<size_t>(st.range(2));
  RunGetAllChunksCase(
      st, format, mode, reader_count,
      VectorConfig(kGetAllChunks2GiBNumRows, true, "vector/2GiB_random", "crt_reader_get_all_chunks_2gib"),
      kGetAllChunks2GiBRawBytes, "get_all_chunks/2GiB_random_vector");
}
...
BENCHMARK_DEFINE_F(CrtReaderBenchmark, ConcurrentOpenGetAllChunksCompressed)(::benchmark::State& st) {
  ...
  const auto reader_count = static_cast<size_t>(st.range(2));
  RunGetAllChunksCase(st, format, mode, reader_count,
                      VectorConfig(kGetAllChunksCompressedNumRows, false, "vector/256MiB_low_entropy",
                                   "crt_reader_get_all_chunks_compressed"),
                      kGetAllChunksCompressedRawBytes, "get_all_chunks/256MiB_low_entropy_vector");
}
