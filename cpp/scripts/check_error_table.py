...
# Renamed aws_* -> storage_*: the six old symbols stay exported in the linker
# maps as tombstones so an older binding loads against a newer library, but
# they are deliberately NOT bound in Python -- nothing produces them and no
# caller should compare against them. They are absent from the X-macro table
# entirely, so the three hand-kept export lists are allowed (and required) to
# disagree about them.
TOMBSTONES = {
...
def parse_header() -> list[dict]:
    ...
    pattern = re.compile(
        ...
        r"LOON_ERROR_CATEGORY_(?P<category>[A-Z_]+)\s*,\s*"
        r'"(?P<s3>[^"]*)"\s*\)'
    )
...
def main() -> int:
    ...
    def taxonomy_only(names):
        return {n for n in names if n.startswith("loon_errcode_") and n not in TOMBSTONES}
