...
1. **Documentation drift.** `docs/error-codes.md` restates every code's
   category, retriability and segcore code. Those columns are
...
# Reserved ABI values must stay exported but must never be produced again.
# Keeping this as a checked tombstone prevents a future catch block from
# quietly reintroducing a second runtime taxonomy for the same failure.
RESERVED_PRODUCERLESS: dict[str, str] = {
    "PackedUnexpected": "retained only for ABI compatibility; new packed internal failures use InternalInvariantViolated",
}


def parse_header() -> list[dict]:
    """Pull names, values, symbols, and categories from the X-macro lists."""
...
def resolve_values(codes: list[dict]) -> None:
    ...
    for c in codes:
        if c["raw_value"].isdigit() or c["raw_value"].lstrip("-").isdigit():
            ...
            sys.exit(
                f"check_error_table: cannot resolve {c['raw_value']} for {c['name']}"
            )
...
def find_producers(codes: list[dict]) -> dict[str, list[str]]:
    ...
    try:
        proc = subprocess.run(
            ...
            [
                ...
                "-n",
                "-E",
                "ExtendStatusCode::|LOON_[A-Z0-9_]+",
                "--",
                "cpp/src",
                "cpp/include",
            ],
    ...
    # name. Both count.
    alias = {
        c["raw_value"]: c["key"] for c in codes if c["raw_value"].startswith("LOON_")
    }
    for line in proc.stdout.splitlines():
        ...
        if re.match(r"\s*case\s+(ExtendStatusCode::|LOON_)", rest):
            continue
        # Comments are not producers either. A `// LOON_MEMORY_ERROR ...`
        # comment sitting next to the real `return` counts as a phantom
        # producer: delete the return and the gate still passes. Strip line
        # comments plus closed and unterminated block comments. (A `//` inside
        # a string literal is also stripped -- acceptable for a gate whose
        # job is to fail conservatively, and no such site exists today.)
        code_only = re.sub(r"//.*", "", rest)
        code_only = re.sub(r"/\*.*?\*/", "", code_only)
        code_only = re.sub(r"/\*.*", "", code_only)
        for m in re.finditer(
            r"(?:ExtendStatusCode::([A-Za-z0-9_]+)|\b(LOON_[A-Z0-9_]+)\b)", code_only
        ):
...
def render_cells(c: dict, segcore: dict[str, str]) -> tuple[str, str, str]:
    cat = {"DATA_FORMAT": "DataFormat"}.get(c["category"], c["category"].capitalize())
    ...
    return cat_cell, retry_cell, seg_cell
...
def main() -> int:
    ...
    # --- 0. no duplicated exported symbol, in any of the three hand-kept lists ---
    #
    ...
    # check came to exist. The three lists cannot be macro-generated (different
    ...
    for rel, pattern in symbol_lists.items():
        ...
        if not path.exists():
            problems.append(
                f"{rel} is missing -- the symbol lists moved without updating this gate"
            )
            continue
    ...
    # The three lists must agree on the TAXONOMY surface. They deliberately do
    ...
    # error codes, which are the data symbols this PR exports. Categories are C
    # enum constants and therefore need no linker or Python symbol entry.
    def taxonomy_only(names):
        return {n for n in names if n.startswith("loon_errcode_")}
    ...
    # that is how LOON_VORTEX_DATA_FORMAT shipped absent from both linker
    ...
    EXPORT_LISTS = (
        ...
        "python/milvus_storage/_ffi.py",
    )
    for c in codes:
        want = f"loon_errcode_{c['symbol']}"
        absent = [
            rel
            for rel in EXPORT_LISTS
            if rel in seen_per_list and want not in seen_per_list[rel]
        ]
        if absent:
            problems.append(
    ...
    # codes it produces. That is a fourth hand-kept list, and the kind this gate
    ...
    header_values = {
        c["raw_value"]: c["value"] for c in codes if c["raw_value"].startswith("LOON_")
    }
    for rs in sorted((ROOT / "cpp/src/format/bridge/rust/src").rglob("*.rs")):
        for name, value in re.findall(
            r"const\s+(LOON_[A-Z0-9_]+)\s*:\s*i32\s*=\s*(\d+)", rs.read_text()
        ):
            expected = header_values.get(name)
            if expected is None:
                problems.append(
                    f"{rs.relative_to(ROOT)} declares {name}, which ffi_error_code.h does not define"
                )
    ...
    # --- 1. live codes need a producer; reserved ABI values must not have one ---
    for c in codes:
        if c["key"] in RESERVED_PRODUCERLESS:
            if producers[c["key"]]:
                problems.append(
                    f"{c['key']} ({c['value']}) is a reserved ABI value but is produced at "
                    f"{sorted(producers[c['key']])}. {RESERVED_PRODUCERLESS[c['key']]}"
                )
            continue
        if producers[c["key"]]:
            continue
    ...
    for c in codes:
        ...
        if len(cells) != expected_cols:
            problems.append(
                f"{c['key']}: doc row has {len(cells)} columns, expected {expected_cols}"
            )
            continue
        cat_cell, retry_cell, seg_cell = render_cells(c, segcore)
        want = list(cells)
        want[1] = str(c["value"])
        ...
        if is_extend:
            if seg_cell == "?":
                problems.append(
                    f"{c['key']}: no ToSegcoreErrorCode case, cannot check the segcore column"
                )
            else:
        ...
        if want != cells:
            ...
            if args.fix:
                ...
                for i, (a, b) in enumerate(zip(cells, want)):
                    if a != b:
                        problems.append(
                            f"{c['key']}: doc says {names[i]}={a!r}, source says {b!r}"
                        )
    ...
    if problems:
        ...
        print(
            "\nThe error taxonomy is an FFI contract. "
    ...
    print(
        f"check_error_table: OK -- {len(codes)} codes ({n_ext} ExtendStatusCode), "
        "live producers and reserved tombstones verified; doc rows in sync"
    )
    return 0
