...
Fixing the sites as they were reported failed: "an OOM was reclassified as
permanent" came back in eleven of thirteen review rounds, at forty-five
separate places, because each fix addressed the instance named and not the
rule.
...
exception is about to be discarded. std::bad_alloc discarded there becomes a
permanent error, which is the one answer guaranteed to be wrong: memory
pressure is precisely the condition a retry can resolve. So the rule is that
such a handler must be preceded by a bad_alloc handler in the same try block.
