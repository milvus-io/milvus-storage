...
/// table and category. Values are the C ABI contract and must never be
/// renumbered.
...
/// can break it.
#define MILVUS_STORAGE_EXTEND_STATUS_RANGE_ASSERT(name, code, symbol, category) \
...
/// Construct a typed error with a producer-selected Arrow transport code.
/// Use this when the same detail code can be detected either before or after IO.
