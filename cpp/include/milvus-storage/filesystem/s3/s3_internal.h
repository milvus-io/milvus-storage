    ...
    case Aws::S3::S3Errors::UNKNOWN:
      ...
      switch (response_code) {
        ...
        case Aws::Http::HttpResponseCode::CONFLICT:
          // 409 is overloaded, and the code says so itself -- BucketNotEmpty
          // and InvalidBucketState are 409s about the bucket, not about a race.
          // This used to blocklist those two and call everything else Conflict,
          // which is a guess about every 409 shape no one here has seen. Now
          // only the names that ARE a lost race are named; the rest stay
          // unclassified, which became a real answer rather than a synonym for
          // permanent.
          if (exception_name == "OperationAborted" || exception_name == "ConditionalRequestConflict" ||
              exception_name == "ConcurrentModification") {
...
std::optional<arrow::Status> tryMakeRetryableExtendArrowError(const Aws::Client::AWSError<ErrorType>& error,
  ...
  // Everything above matched a cause we could name -- an error type, an HTTP
  // status, an exception name. What is left is the residue, and for it the
  // SDK's own retry verdict is the best signal there is: AWS knows which of its
  // failures clear, including ones we did not recognize. That verdict IS a fact
  // about the condition, so the Transient part is kept.
  //
  // What is NOT a fact is the cause. This used to report StorageTransientNetwork
  // here, which named a network problem that nothing established; a test even
  // pinned INTERNAL_FAILURE -- a 500 -- coming back as Network.
  if (error.ShouldRetry()) {
    return MakeExtendError(ExtendStatusCode::StorageTransientUnspecified, message, message /* extra_info */);
  }
...
arrow::Status ErrorToStatus(const std::string& prefix,
  ...
  // Nothing recognized it. It stays unclassified.
  //
  // There used to be one more arm here: ShouldRetry() == false meant Permanent.
  // That inverted the reasoning above -- "the SDK's retry loop declined" is a
  // POLICY, and permanence is a claim about the condition. The two guards it
  // had accumulated say so plainly: S3-compatible backends return genuine
  // transients as UNKNOWN with the non-retryable flag set (MinIO's "SlowDown"
  // arrives exactly that way), and connect errors did too, so each had to be
  // carved out. Every carve-out was a backend the rule was wrong about.
  //
  // Unclassified is now a real answer rather than a synonym for permanent, so
