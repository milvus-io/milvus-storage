...
///
/// Call only from inside a catch block. Everything is LOON_GOT_EXCEPTION --
/// our bug -- except std::bad_alloc, which reports itself as LOON_MEMORY_ERROR.
/// Both are Permanent; the codes differ so that an OOM is still legible AS an
/// OOM in a log or a metric, instead of being filed under "some exception".
///
/// This exists because the two halves of "we ran out of memory" disagreed. An
/// arrow OutOfMemory status mapped to LOON_MEMORY_ERROR; the identical
/// condition arriving as a thrown std::bad_alloc hit the catch-all at every FFI
/// entry point and came back as a bare internal error. Same event, two
/// different names, decided by which layer happened to notice it.
///
/// Note what this does NOT decide. It once made bad_alloc retriable, on the
/// grounds that memory pressure clears -- but the entry points these handlers
/// wrap are not all replayable (some have already committed by the time the
/// allocation fails), and one function cannot know which. Retriability is the
/// category's business, and LOON_MEMORY_ERROR's category is Permanent; see
/// ffi_error_code.h. This function only recovers the type.
///
