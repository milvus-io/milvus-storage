...
// Deliberately ONE code. The earlier 115/116 split claimed to separate operator
// config from user-supplied locations, but the producer cannot tell them apart
// -- on the external-table path the properties AND the URI both come from the
// user's definition, and on the internal path both are generated by milvus.
// The only real difference is WHICH FIELD is bad, which belongs in the message.
