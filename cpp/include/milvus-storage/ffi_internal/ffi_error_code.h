...
//                data, unsupported operation, bug, ...)
//
...
// Legacy AWS-specific names are removed in the public-API cleanup layer.
#define LOON_AWS_ERROR_NO_SUCH_UPLOAD LOON_STORAGE_NO_SUCH_UPLOAD
#define LOON_AWS_ERROR_CONFLICT LOON_STORAGE_CONFLICT
...
#define LOON_INTERNAL_ERROR_CODE_LIST(X)                                                                  \
  ...
  X("Invalid args", LOON_INVALID_ARGS, invalid_args, LOON_ERROR_CATEGORY_SYSTEM)                          \
  ...
  /* incident, so triage lands on the node that could not allocate.             */                        \
  /* Retryable, and the one place this taxonomy admits "the condition may       */                        \
  /* improve" as a transient cause: the request itself is replayable -- nothing */                        \
  /* was submitted, no state moved -- and memory pressure is by nature a        */                        \
  /* passing condition that a retry or a reroute to another replica can clear.  */                        \
  /* Contrast Conflict, which stays non-retryable because replaying the same    */                        \
  /* request is guaranteed to fail again until the caller re-reads and rebases. */                        \
  /* This is also what milvus already believes: merr marks 2034 retriable, so   */                        \
  /* classifying it System here made the two sides of the boundary disagree.    */                        \
  X("Memory allocation failure", LOON_MEMORY_ERROR, memory, LOON_ERROR_CATEGORY_RETRYABLE)                \
  /* Unclassified arrow failure. It carries no transient hint; the operation owner decides recovery. */   \
  X("Internal error", LOON_ARROW_ERROR, arrow, LOON_ERROR_CATEGORY_SYSTEM)                                \
  /* Internal invariant violated -- our bug; retrying reproduces it. */                                   \
  X("Logical error", LOON_LOGICAL_ERROR, logical, LOON_ERROR_CATEGORY_SYSTEM)                             \
  /* Catch-all for a C++ exception escaping an FFI entry point. */                                        \
  X("Got exception", LOON_GOT_EXCEPTION, got_exception, LOON_ERROR_CATEGORY_SYSTEM)                       \
  X("Unreachable code", LOON_UNREACHABLE_ERROR, unreachable, LOON_ERROR_CATEGORY_SYSTEM)                  \
  ...
  X("Invalid properties", LOON_INVALID_PROPERTIES, invalid_properties, LOON_ERROR_CATEGORY_SYSTEM)        \
  ...
  X("Not supported", LOON_NOT_SUPPORT, not_support, LOON_ERROR_CATEGORY_SYSTEM)                           \
  ...
  X("File not found", LOON_FILE_NOT_FOUND, file_not_found, LOON_ERROR_CATEGORY_SYSTEM)                    \
  ...
  /* split cannot be made accurately here. Only entry points contractually handed an            */        \
  /* external source should mint this presentation code.                                        */        \
  X("Source invalid", LOON_SOURCE_INVALID, source_invalid, LOON_ERROR_CATEGORY_SYSTEM)
...
#define LOON_EXTEND_STATUS_CODE_LIST(X)                                                                         \
  /* --- packed layer (50-55) --- */                                                                            \
  X(PackedInvalidArgs, 50, packed_invalid_args, LOON_ERROR_CATEGORY_USER)                                       \
  ...
  X(StorageConflict, LOON_STORAGE_CONFLICT, storage_conflict, LOON_ERROR_CATEGORY_CONFLICT)                     \
  ...
  /* --- our own bugs (122) --- */                                                                              \
  X(InternalInvariantViolated, LOON_INTERNAL_INVARIANT, internal_invariant, LOON_ERROR_CATEGORY_SYSTEM)
