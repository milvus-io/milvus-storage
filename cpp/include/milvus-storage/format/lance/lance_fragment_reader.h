...
class LanceFragmentReader final : public FormatReader, public std::enable_shared_from_this<LanceFragmentReader> {
  public:
  LanceFragmentReader(const std::shared_ptr<BlockingDataset> dataset,
                      uint64_t fragment_id,
                      const std::shared_ptr<arrow::Schema>& schema,
                      const milvus_storage::api::Properties& properties);

  LanceFragmentReader(const std::string& uri,
                      uint64_t fragment_id,
                      const std::shared_ptr<arrow::Schema>& schema,
                      const milvus_storage::api::Properties& properties);
