...
class FormatReaderMetadataCache final {
  static_assert(std::is_base_of<FormatReader, ReaderT>::value, "ReaderT must derive from FormatReader.");
...
class FormatReaderMetadataCaches final {
  ...
  [[nodiscard]] std::shared_ptr<FormatReaderMetadataCache<ReaderT>> get() const {
    static_assert((std::is_same<ReaderT, ReaderTs>::value || ...),
                  "ReaderT must be a supported metadata cache reader type");
