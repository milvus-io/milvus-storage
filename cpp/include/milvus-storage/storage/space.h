...
class Space {
  public:
  arrow::Status Write(arrow::RecordBatchReader& reader, const WriteOption& option);
  ...
  arrow::Status Delete(arrow::RecordBatchReader& reader);
  ...
  static arrow::Result<std::unique_ptr<Space>> Open(const std::string& uri, const Options& options);

  // Write a blob to space. Will return a error if replace is false and a blob with the same name exists.
  arrow::Status WriteBlob(const std::string& name, const void* blob, int64_t length, bool replace = false);

  // Read a blob from space, the target must have enough size to hold this blob.
  arrow::Status ReadBlob(const std::string& name, void* target) const;

  // Get the byte size of a blob.
  arrow::Result<int64_t> GetBlobByteSize(const std::string& name) const;
  ...
  static arrow::Status SafeSaveManifest(arrow::fs::FileSystem& fs, const std::string& path, const Manifest& manifest);

  static arrow::Result<arrow::fs::FileInfoVector> FindAllManifest(arrow::fs::FileSystem& fs, const std::string& path);
