...
static arrow::Result<std::vector<ColumnGroupFile>> ExploreIceberg(
  ...
  ARROW_ASSIGN_OR_RAISE(auto snapshot_str,
      GetValue<std::string>(properties, "iceberg.snapshot_id"));
...
static arrow::Result<std::shared_ptr<Manifest>> ReadManifest(
  ...
  ARROW_ASSIGN_OR_RAISE(auto input_file, fs->OpenInputFile(manifest_path));
  ARROW_ASSIGN_OR_RAISE(auto file_size, input_file->GetSize());
  ARROW_ASSIGN_OR_RAISE(auto buf, input_file->Read(file_size));
