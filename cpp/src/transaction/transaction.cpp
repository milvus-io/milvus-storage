...
arrow::Status TransactionImpl<T>::begin() {
  ...
  ARROW_ASSIGN_OR_RAISE(current_manifest_, handler_->get_manifest(read_version_));
  ...
  switch (resolve) {
    ...
    case RESOLVE_MERGE: {
      ...
      while (!commit_result && num_retries > 0) {
        ...
        ARROW_ASSIGN_OR_RAISE(auto latest_manifest, handler_->get_manifest(latest_version));
