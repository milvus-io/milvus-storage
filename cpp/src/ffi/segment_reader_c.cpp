...
LoonFFIResult loon_segment_reader_open(const char* segment_path,
  ...
  try {
    auto columns_result = convert_needed_columns(needed_columns, static_cast<size_t>(num_columns));
    RETURN_ARROW_ERROR_IF(columns_result.status(), LOON_INVALID_ARGS, columns_result.status().ToString());
    ...
    auto converted_columns = std::move(columns_result).ValueOrDie();
    if (converted_columns) {
      columns = std::move(*converted_columns);
    }
...
LoonFFIResult loon_segment_reader_get_stream(LoonSegmentReaderHandle handle, ArrowArrayStream* out_stream) {
  ...
  try {
    ...
    auto result = reader->GetStream();
    RETURN_ARROW_ERROR_IF(result.status(), LOON_ARROW_ERROR, result.status().ToString());

    auto status = arrow::ExportRecordBatchReader(std::move(result).ValueOrDie(), out_stream);
...
LoonFFIResult loon_segment_reader_get_chunk_reader(LoonSegmentReaderHandle handle,
  ...
  try {
    auto columns_result = convert_needed_columns(needed_columns, num_columns);
    RETURN_ARROW_ERROR_IF(columns_result.status(), LOON_INVALID_ARGS, columns_result.status().ToString());
    ...
    auto columns = std::move(columns_result).ValueOrDie();
