...
arrow::Status ValidateColumnGroups(const std::vector<std::vector<std::string>>& columns_per_group,
                                   const std::vector<std::vector<std::string>>& files_per_group,
                                   const std::vector<std::vector<int64_t>>& row_counts_per_group,
                                   const std::string& format) {
  if (format.empty()) {
    return arrow::Status::Invalid("format must not be empty");
  }
  const size_t num_groups = columns_per_group.size();
  if (num_groups != files_per_group.size() || num_groups != row_counts_per_group.size()) {
    return arrow::Status::Invalid("per-group array length mismatch: cols=", num_groups,
                                  ", files=", files_per_group.size(), ", rowCounts=", row_counts_per_group.size());
  }
  if (num_groups == 0) {
    return arrow::Status::Invalid("at least one column group is required");
  }
  for (size_t g = 0; g < num_groups; ++g) {
    ...
    if (cols.empty() || files.empty()) {
      return arrow::Status::Invalid("group[", g, "]: columns/files must be non-empty");
    }
    if (rcs.size() != files.size()) {
      return arrow::Status::Invalid("group[", g, "]: rowCounts.length (", rcs.size(), ") != files.length (",
                                    files.size(), ")");
    }
    for (const auto row_count : rcs) {
      if (row_count < 0) {
        return arrow::Status::Invalid("file row counts must be nonnegative");
      }
    }
  }
...
LoonColumnGroups* BuildLoonColumnGroups(const std::vector<std::vector<std::string>>& columns_per_group,
                                        ...
                                        const std::vector<std::vector<int64_t>>& row_counts_per_group,
                                        const std::string& format) {
  const auto status = ValidateColumnGroups(columns_per_group, files_per_group, row_counts_per_group, format);
  if (!status.ok()) {
