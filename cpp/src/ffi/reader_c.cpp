...
AliveReadOptions ConvertAliveReadOptions(const LoonAliveReadOptions* options) {
  AliveReadOptions cpp_options;
  if (options == nullptr) {
    return cpp_options;
  }
  cpp_options.batch_size = options->batch_size;
  cpp_options.parallelism = options->parallelism == 0 ? 1 : options->parallelism;
...
LoonFFIResult loon_alive_reader_new(const LoonManifest* manifest,
  ...
  try {
    ...
    auto stream_result = reader->get_masked_record_batch_reader(ConvertAliveReadOptions(options));
    if (!stream_result.ok()) {
