...
LoonFFIResult loon_reader_new(const LoonColumnGroups* column_groups,
  ...
  try {
    auto cpp_needed_columns_result = convert_needed_columns(needed_columns, num_columns);
    RETURN_ARROW_ERROR_IF(cpp_needed_columns_result.status(), LOON_INVALID_ARGS,
                          cpp_needed_columns_result.status().ToString());
    auto cpp_needed_columns = std::move(cpp_needed_columns_result).ValueOrDie();
    ...
    RETURN_ARROW_ERROR_IF(import_st, LOON_INVALID_ARGS, import_st.ToString());
