...
// ==================== Per-batch RecordBatchReader API ====================
//
// ArrowArrayStream-based iteration (`loon_get_record_batch_reader` above)
// turned out to be incompatible with the Arrow Java `ArrowReader`'s
// shared-VectorSchemaRoot pattern: when the underlying reader emits
// `RecordBatch::Slice` results whose ArrowArray carries a non-zero
// `offset`, the JVM side ignores the offset and keeps reading from buffer
// position 0 — every batch after the first shows the same data. Milvus's
// Go binding (`internal/core/src/segcore/packed_reader_c.cpp ReadNext`)
// sidesteps this by exporting a fresh ArrowArray+ArrowSchema pair per
// batch. The functions below provide the same shape for the Scala/JNI
// path so the Spark connector can mirror Milvus's proven scheme.
//
// Lifecycle:
...
//     ArrowArray/ArrowSchema structs (allocated + zero-initialized by the
//     caller) for each batch; on EOF the `release` fields are left NULL.
...
LoonFFIResult loon_record_batch_reader_read_next(LoonRecordBatchReaderHandle handle,
  ...
  try {
    ...
    // Java's `Data.importVectorSchemaRoot` ignores it. Exporting a sliced
    // batch to a Java consumer therefore produces a row-cycle: every
    // post-slice batch re-reads from buffer position 0, duplicating the
    // first min_rows rows' data.
    //
    // Materialize sliced columns into fresh offset=0 arrays via
    // arrow::Concatenate (allocates new buffers, copies only the slice
    // range). Non-sliced columns pass through unchanged.
    if (batch != nullptr) {
    ...
    if (batch == nullptr) {
      // EOF: leave release callbacks NULL so the caller can detect it.
