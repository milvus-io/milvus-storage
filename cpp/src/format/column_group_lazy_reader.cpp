...
arrow::Result<std::shared_ptr<arrow::Table>> ColumnGroupLazyReaderImpl::take(const std::vector<int64_t>& row_indices) {
  ...
  std::vector<int64_t> sorted_row_indices(row_indices);
  std::sort(sorted_row_indices.begin(), sorted_row_indices.end());
  sorted_row_indices.erase(std::unique(sorted_row_indices.begin(), sorted_row_indices.end()), sorted_row_indices.end());

  // if row_indices is not sorted or not unique, we need to reorder the result
  bool need_reorder = (row_indices.size() != sorted_row_indices.size() || row_indices != sorted_row_indices);

  const auto& cg_files = column_group_->files;
  std::vector<std::vector<int64_t>> indices_in_files(cg_files.size());

  for (const auto& row_index : sorted_row_indices) {
    uint32_t file_index;
