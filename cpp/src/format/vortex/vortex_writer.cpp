...
VortexFileWriter::VortexFileWriter(ArrowFileSystemConfig config,
                                   ...
                                   const api::WriteProperties& properties)
...
arrow::Status VortexFileWriter::Flush() {
  ...
  if (!obj_store_) {
    ...
    if (rc != C_SUCCESS) {
      return arrow::Status::Invalid("Faile to init the object store in rust. rc: " + std::to_string(rc));
    }
  }
  ...
  if (rc != C_SUCCESS) {
    return arrow::Status::Invalid("Faile to write array stream in rust. rc: " + std::to_string(rc));
  }
