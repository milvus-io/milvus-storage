...
arrow::Result<std::unique_ptr<arrow::RecordBatchReader>> VortexFormatReader::read_with_range(
    const uint64_t& start_offset, const uint64_t& end_offset) {
  std::vector<std::shared_ptr<arrow::RecordBatch>> result;

  assert(vxfile_);
  ARROW_ASSIGN_OR_RAISE(auto chunkedarray, read(start_offset, end_offset));
  assert(chunkedarray);
  ...
  ARROW_ASSIGN_OR_RAISE(auto table, arrow::Table::FromRecordBatches(result));
  return std::make_unique<arrow::TableBatchReader>(table);
}
