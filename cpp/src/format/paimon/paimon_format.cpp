...
arrow::Result<std::vector<api::ColumnGroupFile>> PaimonFormat::explore(const std::string& explore_dir,
  ...
  for (const auto& info : planned) {
    if (info.route_reason.empty()) {
      return arrow::Status::Invalid("Paimon planner returned an empty route reason");
    }
    int64_t record_count = -1;
    try {
      auto metadata = folly::parseJson(info.metadata_json);
      auto canonical = metadata.getDefault("read_path", "").asString();
      if (canonical != info.read_path) {
        return arrow::Status::Invalid("Paimon planning read_path mismatch: metadata=", canonical,
                                      ", bridge=", info.read_path);
      }
    ...
    LOG_STORAGE_DEBUG_ << "Paimon planned read_path=" << info.read_path << ", reason=" << info.route_reason
                       << ", rows=" << record_count;
    ...
    if (info.read_path == "direct-file" && info.file_size > 0) {
