...
/// Captures caller context by wrapping a Reader separately for each submission.
/// The shared scheduler retains only this function, never a caller's context.
pub type RequestReaderWrapper = fn(Arc<dyn Reader>) -> Arc<dyn Reader>;
...
pub struct ScanScheduler {
    request_reader_wrapper: Option<RequestReaderWrapper>,
...
impl ScanScheduler {
    ...
    pub fn new(object_store: Arc<ObjectStore>, config: SchedulerConfig) -> Arc<Self> {
        Self::new_with_reader_wrapper(object_store, config, None)
    }

    pub fn new_with_reader_wrapper(
        object_store: Arc<ObjectStore>,
    ...
    pub fn submit_request(
        ...
        let reader = match self.request_reader_wrapper {
