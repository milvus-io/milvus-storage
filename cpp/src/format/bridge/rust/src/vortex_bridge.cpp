...
VortexFile VortexFile::Open(uint8_t* fs_rawptr, const std::string& path, uint64_t file_size) {
  try {
    return VortexFile(ffi::open_file(fs_rawptr, rust::Str(path.data(), path.length()), file_size));
  } catch (const rust::cxxbridge1::Error& e) {
    throw VortexException(e.what());
  }
}

std::unique_ptr<VortexFile> VortexFile::OpenUnique(uint8_t* fs_rawptr, const std::string& path, uint64_t file_size) {
  try {
    return std::unique_ptr<VortexFile>(
        new VortexFile(ffi::open_file(fs_rawptr, rust::Str(path.data(), path.length()), file_size)));
  } catch (const rust::cxxbridge1::Error& e) {
    throw VortexException(e.what());
  }
}
