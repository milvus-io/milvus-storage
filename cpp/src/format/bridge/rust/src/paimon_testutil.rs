...
fn fixture_schema(mode: &str, file_format: &str, dimension: u32) -> Result<TableSchema> {
    let file_format = if mode == "vortex" {
        "vortex"
    } else {
        file_format
    };
    ...
    match mode {
        ...
        "vortex" => {
            builder = builder.option("file.format", "vortex");
        }
        other => bail!(
            "unsupported Paimon fixture mode '{other}'; expected append, mor, deletion-vector, or vortex"
        ),
    }
...
pub fn paimon_create_test_table(
    ...
    TOKIO_RT.block_on(async move {
        ...
        let snapshot_id = *snapshot_ids.last().unwrap_or(&0);
        Ok(PaimonTestTableInfo {
