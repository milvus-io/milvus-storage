...
std::shared_ptr<BlockingDataset> BlockingDataset::Open(const std::string &uri) {
    try {
        return std::make_shared<BlockingDataset>(ffi::open_dataset(
            rust::Str(uri.data(), uri.length())
            ));
...
std::unique_ptr<BlockingDataset> BlockingDataset::OpenUnique(const std::string &uri) {
    try {
        return std::make_unique<BlockingDataset>(ffi::open_dataset(
            rust::Str(uri.data(), uri.length())
            ));
    } catch (const rust::cxxbridge1::Error &e) {
        throw LanceException(e.what());
...
std::unique_ptr<BlockingDataset> BlockingDataset::WriteDataset(const std::string &uri, 
    ...
    try {
        return std::make_unique<BlockingDataset>(ffi::write_dataset(
            rust::Str(uri.data(), uri.length()), reinterpret_cast<uint8_t *>(stream)));
    } catch (const rust::cxxbridge1::Error &e) {
        throw LanceException(e.what());
    }
}
...
std::unique_ptr<BlockingFragmentReader> BlockingFragmentReader::Open(
    const BlockingDataset &dataset,
...
void BlockingFragmentReader::TakeAsSingleBatch(
...
ArrowArrayStream BlockingFragmentReader::ReadRangesAsStream(
    uint32_t row_range_start,
