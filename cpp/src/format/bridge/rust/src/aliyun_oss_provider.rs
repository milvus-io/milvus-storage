...
//!     signing reused by both RAM mode and OIDC step 2.
//!
...
fn remove_implicit_role_fields(config_map: &mut HashMap<String, String>) {
...
fn init_oss_env_config(bucket: String, base_path: &Url) -> HashMap<String, String> {
    ...
    // `oidc_token_file` / `oidc_provider_arn` via this path.
    ...
    // In this bridge `role_arn` means the caller-supplied tenant target role.
    // Role fields inherited from process env are ambiguous:
    // `ALIBABA_CLOUD_ROLE_ARN` is the machine-identity role used by step 1 of
    // the OIDC chain, while `OSS_ROLE_ARN` / `AWS_ROLE_ARN` would be global
    // target roles. Only explicit storage options below may set tenant role
    // fields; otherwise plain AK/SK Iceberg writes can accidentally attempt
    // the two-step AssumeRole flow.
    remove_implicit_role_fields(&mut config_map);
...
/// `oss.region`, `oss.role-arn`, `oss.role-session-name`, `oss.external-id`.
///
/// The three endpoint/AK/SK keys match iceberg core's `OSS_*` constants
/// (`OSS_ENDPOINT`, `OSS_ACCESS_KEY_ID`, `OSS_ACCESS_KEY_SECRET`). The role
/// keys are custom to this project — stock iceberg-storage-opendal's
...
pub(crate) fn build_oss_config_from_iceberg_opts(
    ...
    if let Some(external_id) = storage_options.get("oss.external-id") {
        config_map.insert("external_id".to_string(), external_id.clone());
    }
...
async fn apply_oidc_chain_if_requested_with_expiration(
    ...
    let outer = ram::call_assume_role(
        &client,
        &inner,
        &target_role_arn,
        &session_name,
...
/// opendal config via [`build_oss_config_from_iceberg_opts`] plus the local
/// Aliyun credential chain so per-tenant `oss.role-arn`, OIDC chaining, and
/// the ECS-IMDS RAM flow are honoured (stock iceberg-storage-opendal's
/// `OssConfig` cannot carry them).
...
pub(crate) mod ram {
    ...
    pub(super) async fn call_assume_role(
        ...
        let parsed: AssumeRoleResponseJson = serde_json::from_str(&text)
            .map_err(|e| format!("sts:AssumeRole JSON parse failed ({e}); body was: {text}"))?;
    ...
    mod tests {
        ...
        fn apply_credentials_strips_role_arn_and_injects_static_creds() {
            ...
            assert_eq!(
                cfg.get("access_key_secret").map(String::as_str),
                Some("AKSK")
            );
        ...
        fn pop_encode_unreserved_set() {
            ...
            assert_eq!(
                pop_encode("acs:ram::1:role/x"),
                "acs%3Aram%3A%3A1%3Arole%2Fx"
            );
        }
        ...
        fn pop_sign_known_vector() {
            ...
            let canonical = "AccessKeyId=testid&Action=DescribeRegions&Format=XML&SignatureMethod=HMAC-SHA1\
