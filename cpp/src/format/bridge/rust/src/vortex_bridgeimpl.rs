...
///
/// Both types have identical memory layout:
/// - FixedSizeBinary(N): [validity_buffer, data_buffer] where data is N bytes per element
/// - FixedSizeList<u8, N>: [validity_buffer, child_data] where child is N u8 values per element
///
/// We share the underlying buffer via Arc (Buffer::clone is O(1)).
fn convert_fixed_size_binary_to_list(array: &FixedSizeBinaryArray) -> ArrowArrayRef {
    ...
    let values_buffer = data.buffers()[0].clone(); // Buffer::clone() is Arc-like, O(1)
...
///
/// Reverse of convert_fixed_size_binary_to_list.
/// We share the underlying buffer via Arc (Buffer::clone is O(1)).
fn convert_list_to_fixed_size_binary(array: &FixedSizeListArray, byte_width: i32) -> ArrowArrayRef {
    ...
    let values_buffer = u8_data.buffers()[0].clone(); // Buffer::clone() is Arc-like, O(1)
...
/// Convert RecordBatch: replace FixedSizeBinary columns with FixedSizeList<u8>
fn convert_record_batch_for_vortex(batch: &RecordBatch) -> RecordBatch {
    if !schema_has_fixed_size_binary(batch.schema().as_ref()) {
        return batch.clone();
    }

    let new_schema = Arc::new(convert_schema_for_vortex(batch.schema().as_ref()));
...
/// A cloneable wrapper that converts FixedSizeList<u8> back to FixedSizeBinary
struct ConvertingCloneableReader<T: ThreadsafeCloneableReaderTrait> {
    inner: T,
    original_schema: SchemaRef,
}

impl<T: ThreadsafeCloneableReaderTrait> Clone for ConvertingCloneableReader<T>
where
    T: Clone,
{
    fn clone(&self) -> Self {
...
impl<T: ThreadsafeCloneableReaderTrait + Clone> std::iter::Iterator for ConvertingCloneableReader<T> {
    type Item = Result<RecordBatch, ArrowError>;
...
impl<T: ThreadsafeCloneableReaderTrait + Clone> RecordBatchReader for ConvertingCloneableReader<T> {
    fn schema(&self) -> SchemaRef {
        self.original_schema.clone()
    }
}
...
pub(crate) fn scan_builder_into_threadsafe_cloneable_reader(
    ...
    let has_fsb = schema_has_fixed_size_binary(&original_schema);
    let original_schema_clone = original_schema.clone();
    ...
    let stream = builder
        ...
        .map(move |b| {
            b.into_arrow(&data_type)
                .map(|struct_array| {
                    let batch = RecordBatch::from(struct_array.as_struct());
                    // Convert back if needed
                    if has_fsb {
                        convert_record_batch_from_vortex(&batch, &original_schema_clone)
                    } else {
                        batch
                    }
                })
    ...
    let rbr = RecordBatchIteratorAdapter::new(iter, original_schema);
