...
LanceFragmentReader::LanceFragmentReader(const std::shared_ptr<BlockingDataset> dataset,
...
LanceFragmentReader::LanceFragmentReader(const std::string& uri,
                                         uint64_t fragment_id,
                                         const std::shared_ptr<arrow::Schema>& schema,
                                         const milvus_storage::api::Properties& properties)
...
arrow::Status LanceFragmentReader::open() {
  ...
  ARROW_ASSIGN_OR_RAISE(logical_chunk_rows_,
                        api::GetValue<uint64_t>(properties_, PROPERTY_READER_LANCE_FRAGMENT_CHUNK_ROWS));
...
arrow::Result<std::vector<RowGroupInfo>> LanceFragmentReader::get_row_group_infos() {
  assert(fragment_reader_);
  return row_group_infos_;
}

arrow::Result<std::shared_ptr<arrow::RecordBatch>> LanceFragmentReader::get_chunk(const int& row_group_index) {
  assert(fragment_reader_);
...
arrow::Result<std::vector<std::shared_ptr<arrow::RecordBatch>>> LanceFragmentReader::get_chunks(
...
arrow::Result<std::shared_ptr<arrow::Table>> LanceFragmentReader::take(const std::vector<int64_t>& row_indices) {
  assert(fragment_reader_);
...
arrow::Result<std::shared_ptr<arrow::RecordBatchReader>> LanceFragmentReader::read_with_range(
...
arrow::Result<std::shared_ptr<FormatReader>> LanceFragmentReader::clone_reader() {
