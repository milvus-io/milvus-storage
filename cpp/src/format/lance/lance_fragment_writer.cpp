...
LanceFragmentWriter::LanceFragmentWriter(const std::string& base_path,
...
arrow::Status LanceFragmentWriter::Flush() { return arrow::Status::OK(); }
...
arrow::Result<api::ColumnGroupFile> LanceFragmentWriter::Close() {
  assert(!closed_);
  ...
  if (current_fids.size() < origin_fids_.size()) {
    return arrow::Status::Invalid(
        "LanceFragmentWriter: current fragment ids size is less than origin fragment ids size");
  }
  ...
  if (!fids_contains(origin_fids_, current_fids)) {
    return arrow::Status::Invalid("LanceFragmentWriter: current fragment ids is not a superset of origin fragment ids");
  }

  append_fids = fids_diff(origin_fids_, current_fids);
  // must be 1 in gtest
  assert(append_fids.size() == 1);
  const size_t byte_size = append_fids.size() * sizeof(uint64_t);
  std::vector<uint8_t> output(byte_size);
  memcpy(output.data(), append_fids.data(), byte_size);
