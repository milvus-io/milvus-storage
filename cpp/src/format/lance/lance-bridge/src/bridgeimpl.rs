...
impl BlockingFragmentReader {
    pub fn open(
        ...
        let projection = Schema::try_from(arrow_projection)?;

        let dataset_cloned = Arc::new(dataset.inner.clone());
        let fragment = FileFragment::new(dataset_cloned, fragment);
        let fragment_reader = RT.block_on(fragment.open(&projection, read_config))?;
    ...
    pub unsafe fn take_as_stream(
        ...
        let ffi_stream = read_batch_fut_stream?.to_ffi_stream(
            Arc::new(ArrowSchema::from(self.fragment.schema())),
            RT.handle().clone(),
        );
