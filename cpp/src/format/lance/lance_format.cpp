...
arrow::Result<std::vector<api::ColumnGroupFile>> LanceFormat::explore(const std::string& explore_dir,
  ...
  auto dataset = lance::BlockingDataset::Open(lance_base_uri, storage_options, requested_version);
...
arrow::Result<std::shared_ptr<FormatReader>> LanceFormat::create_reader(
  ...
  auto reader = std::make_shared<lance::LanceTableReader>(base_path, fragment_id, read_schema, properties,
