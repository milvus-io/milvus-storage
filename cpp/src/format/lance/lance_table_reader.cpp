...
arrow::Result<LanceTableReader::MetaTrait::MetadataPtr> LanceTableReader::MetaTrait::load_metadata(
  ...
  ARROW_ASSIGN_OR_RAISE(std::shared_ptr<BlockingDataset> dataset,
                        BlockingDataset::Open(lance_uri, ToStorageOptions(fs_config)));
...
arrow::Result<std::shared_ptr<LanceTableReader>> LanceTableReader::MetaTrait::create_from_metadata(
  ...
  auto fragment_reader_result =
      BlockingFragmentReader::Open(*metadata->payload.dataset, metadata->payload.fragment_id, c_arrow_schema);
  if (!fragment_reader_result.ok()) {
    if (c_arrow_schema.release) {
      c_arrow_schema.release(&c_arrow_schema);
    }
    return milvus_storage::bridge::WithBridgeContext(
        fmt::format("Failed to open Lance fragment reader for fragment {}", metadata->payload.fragment_id),
        fragment_reader_result.status());
  }
  reader->fragment_reader_ = std::move(*fragment_reader_result);
...
arrow::Status LanceTableReader::open() {
  ...
  {
    auto fragment_reader_result = BlockingFragmentReader::Open(*dataset_, fragment_id_, c_arrow_schema);
    if (!fragment_reader_result.ok()) {
      if (c_arrow_schema.release) {
        c_arrow_schema.release(&c_arrow_schema);
      }
      return milvus_storage::bridge::WithBridgeContext(
          fmt::format("Failed to open Lance fragment reader for fragment {}", fragment_id_),
          fragment_reader_result.status());
    }
    fragment_reader_ = std::move(*fragment_reader_result);
  }
  ...
  ARROW_ASSIGN_OR_RAISE(uint64_t logical_rows, fragment_reader_->RowCount());
  {
    ARROW_ASSIGN_OR_RAISE(uint64_t physical_rows, dataset_->GetFragmentPhysicalRowCount(fragment_id_));
    if (physical_rows < logical_rows) {
...
arrow::Result<std::shared_ptr<arrow::RecordBatch>> LanceTableReader::get_chunk(const int& row_group_index) {
  ...
  ARROW_ASSIGN_OR_RAISE(ArrowArrayStream array_stream,
                        fragment_reader_->ReadRangesAsStream(start_idx, end_idx, end_idx - start_idx + num_deletions_));
  // ImportChunkedArray drains the stream: mid-scan bridge errors surface here
  // as raw marker-encoded strings and must be decoded (see bridge_error.h).
  auto chunked_result = arrow::ImportChunkedArray(&array_stream);
  if (!chunked_result.ok()) {
    return milvus_storage::bridge::TranslateBridgeStatus("Failed to read lance chunk", chunked_result.status());
  }
  auto chunkedarray = std::move(*chunked_result);
...
arrow::Result<std::vector<std::shared_ptr<arrow::RecordBatch>>> LanceTableReader::get_chunks(
  ...
  for (const auto& rg_range : rg_idx_ranges) {
    ...
    auto chunked_result = arrow::ImportChunkedArray(&array_stream);
    if (!chunked_result.ok()) {
      return milvus_storage::bridge::TranslateBridgeStatus("Failed to read lance chunks", chunked_result.status());
    }
    auto chunkedarray = std::move(*chunked_result);
...
arrow::Result<std::shared_ptr<arrow::Table>> LanceTableReader::take(const std::vector<int64_t>& row_indices) {
  assert(fragment_reader_);
  ARROW_ASSIGN_OR_RAISE(ArrowArrayStream array_stream, fragment_reader_->TakeAsStream(row_indices, row_indices.size()));
  auto chunked_result = arrow::ImportChunkedArray(&array_stream);
  if (!chunked_result.ok()) {
    return milvus_storage::bridge::TranslateBridgeStatus("Failed to take lance rows", chunked_result.status());
  }
  auto chunkedarray = std::move(*chunked_result);
...
arrow::Result<std::shared_ptr<arrow::RecordBatchReader>> LanceTableReader::read_with_range(const uint64_t& start_offset,
  ...
  // The stream stays live in the caller: wrap it so mid-scan bridge errors are
  // decoded on every ReadNext instead of surfacing as raw marker strings.
  return milvus_storage::bridge::WrapBridgeRecordBatchReader(std::move(reader), "Failed to read lance record batch");
}
