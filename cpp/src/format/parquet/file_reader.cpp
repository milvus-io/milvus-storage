...
arrow::Status FileRecordBatchReader::SetRowGroupOffsetAndCount(size_t row_group_offset, size_t row_group_num) {
  size_t row_group_end = row_group_offset + row_group_num;
  if (row_group_offset >= row_group_sizes_.size() || row_group_end > row_group_sizes_.size()) {
  ...
  current_row_group_ = row_group_offset;
  row_group_end_ = row_group_end;
