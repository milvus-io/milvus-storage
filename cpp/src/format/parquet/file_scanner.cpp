...
void ParquetFileScanner::InitRecordReader(parquet::arrow::FileReader *reader,
  ...
  for (int i = 0; i < metadata->num_row_groups(); ++i) {
    ...
    for (const auto &filter : options->filters) {
      auto column_idx =
          metadata->schema()->ColumnIndex(filter->get_column_name());
  ...
  auto status = reader->GetRecordBatchReader(row_group_indices, column_indices,
                                             &record_reader_);
  if (!status.ok()) {
...
std::shared_ptr<arrow::RecordBatch> ParquetFileScanner::Read() {
  ...
  if (!res.ok()) {
    throw StorageException("read record failed");
  }
