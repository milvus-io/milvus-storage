...
#define PARQUET_FOOTER_TRAILER_SIZE 8  // footer_length(4B) + magic(4B)
  ...
  if (std::memcmp(data + suffix->size() - PARQUET_MAGIC_SIZE, PARQUET_MAGIC, PARQUET_MAGIC_SIZE) != 0 ||
      static_cast<uint64_t>(footer_length) > static_cast<uint64_t>(suffix->size() - PARQUET_FOOTER_TRAILER_SIZE)) {
    return nullptr;
  }
...
#define PARQUET_FOOTER_TRAILER_SIZE 8  // footer_length(4B) + magic(4B)
  ...
  uint32_t footer_length = 0;
  std::memcpy(&footer_length, data + suffix->size() - PARQUET_FOOTER_TRAILER_SIZE, sizeof(footer_length));
  if (static_cast<uint64_t>(footer_length) > static_cast<uint64_t>(suffix->size() - PARQUET_FOOTER_TRAILER_SIZE)) {
    return nullptr;
  }
...
folly::SemiFuture<arrow::Status> ParquetFormatReader::open_async() {
  ...
  return folly::makeSemiFuture().deferExValue([self = std::move(self)](
    ...
    auto maybe_executor = MakeFollyArrowExecutor(std::move(executor));
    if (!maybe_executor.ok()) {
      return folly::makeSemiFuture(maybe_executor.status());
    }
    ...
    auto state = std::make_shared<AsyncParquetOpenState>(AsyncParquetOpenState{
        .reader = self,
        .file = std::move(file),
        .executor = std::move(maybe_executor).ValueUnsafe(),
    ...
    auto prepare_metadata = [state](int64_t file_size) -> arrow::Future<std::shared_ptr<::parquet::FileMetaData>> {
      ...
      return read_file_async_and_transfer(state->file, file_size - footer_size, footer_size, state->executor)
          .Then([state](const std::shared_ptr<arrow::Buffer>& footer_buffer)
            ...
            const auto metadata_length = trailer.metadata_length;
            return read_file_async_and_transfer(state->file, metadata_offset, trailer.metadata_length, state->executor)
                .Then([state, metadata_length](const std::shared_ptr<arrow::Buffer>& metadata_buffer) {
                  return try_parse_plain_footer_metadata(metadata_buffer, metadata_length, state->reader_properties);
                });
          });
    };

    // Arrow 17's ParquetFileReader::OpenAsync constructs SerializedFile and
    // synchronously calls GetSize(). The size is ready before that happens:
    //   1. A usable known size (> 8) was passed to OpenInputFile(FileInfo), so
    //      the file already has the size and no async lookup is needed here.
    //   2. Otherwise, resolve the size first. Current remote file implementations
    //      cache it before this future completes; only then do footer preparation
    //      and Parquet open continue, so Parquet's GetSize() reads the cache.
