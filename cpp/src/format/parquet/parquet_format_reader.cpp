...
static arrow::Result<std::vector<RowGroupInfo>> try_build_row_group_infos(
  ...
  ARROW_RETURN_NOT_OK(row_group_metadatas.ValidateAgainst(*metadata));
  row_group_infos.reserve(row_group_metadatas.size());
  size_t offset = 0;
  for (size_t i = 0; i < row_group_metadatas.size(); ++i) {
    const auto& row_group_metadata = row_group_metadatas.Get(i);
    const auto row_count = static_cast<size_t>(row_group_metadata.row_num());
...
#define PARQUET_FOOTER_TRAILER_SIZE 8  // footer_length(4B) + magic(4B)
  ...
  if (auto* async_file = dynamic_cast<milvus_storage::NonBlockingReadAtFile*>(file.get())) {
    ...
    auto read_future = async_file->ReadAtAsyncInto(offset, size, buf->mutable_data());
    ARROW_ASSIGN_OR_RAISE(const auto bytes_read, read_future.result());
    if (bytes_read != size) {
...
arrow::Result<std::shared_ptr<ParquetFormatReader>> ParquetFormatReader::MetaTrait::create_from_metadata(
  ...
  if (!metadata) {
    return MakeExtendErrorMsg(ExtendStatusCode::InternalInvariantViolated,
                              "Cannot open parquet reader from null metadata");
  }
