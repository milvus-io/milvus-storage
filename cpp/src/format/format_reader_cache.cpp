...
arrow::Result<typename FormatReaderMetadataCache<ReaderT>::MetadataPtr> FormatReaderMetadataCache<ReaderT>::get_or_open(
  ...
  try {
    ...
    if (load_result.ok()) {
      ...
      if (!metadata) {
        status = MakeExtendErrorMsg(ExtendStatusCode::InternalInvariantViolated,
                                    "Format reader metadata loader returned null metadata");
      }
    }
  ...
  return folly::makeSemiFuture().deferValue([self = std::move(self), key,
    ...
    try {
      return load_fn().defer(
          [self, key, in_flight_load](folly::Try<MetadataResult>&& load_try) noexcept -> MetadataResult {
