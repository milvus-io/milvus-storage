...
DeleteSet::DeleteSet(const DefaultSpace& space) : space_(space) {
  auto arrow_schema = space_.schema_->schema();
  auto schema_options = space.schema_->options();

  auto pk_type = arrow_schema->GetFieldByName(schema_options->primary_column)->type();
  ...
  auto version_type = arrow_schema->GetFieldByName(schema_options->version_column)->type();
  if (version_type->id() != arrow::Type::INT64) {
    throw StorageException("version type must be int64");
  }

  const auto& delete_files = space.manifest_->delete_files();
