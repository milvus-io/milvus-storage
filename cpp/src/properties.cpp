...
static std::unordered_map<std::string, PropertyInfo> property_infos = {
    ...
    REGISTER_PROPERTY(PROPERTY_WRITER_VORTEX_ENABLE_STATISTICS,
                      ...
                      false,
                      ValidatePropertyType()),
    REGISTER_PROPERTY(PROPERTY_WRITER_VORTEX_SEGMENT_ROW_SIZE,
                      PropertyType::UINT64,
                      "The number of rows per segment for scalar columns in Vortex writer.",
                      uint64_t(8192),
                      ValidatePropertyType() + ValidatePropertyRange<uint64_t>(1, UINT64_MAX)),
    REGISTER_PROPERTY(PROPERTY_WRITER_VORTEX_VECTOR_SEGMENT_ROW_SIZE,
                      PropertyType::UINT64,
                      "The number of rows per segment for vector columns (FixedSizeList) in Vortex writer.",
                      uint64_t(256),
                      ValidatePropertyType() + ValidatePropertyRange<uint64_t>(1, UINT64_MAX)),
    REGISTER_PROPERTY(PROPERTY_WRITER_VORTEX_VARLEN_SEGMENT_ROW_SIZE,
                      PropertyType::UINT64,
                      "The number of rows per segment for variable-length columns (Binary/List/Utf8) in Vortex writer.",
                      uint64_t(2048),
                      ValidatePropertyType() + ValidatePropertyRange<uint64_t>(1, UINT64_MAX)),
