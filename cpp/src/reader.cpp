...
class ReaderImpl : public Reader {
  ...
  [[nodiscard]] arrow::Result<std::shared_ptr<MaskedRecordBatchReader>> get_masked_record_batch_reader(
    ...
    auto metadata_cache = get_metadata_cache();
    auto base_reader = std::make_shared<PackedRecordBatchReader>(
        needed_column_groups, std::move(metadata_cache), projected_schema, resolved_columns, properties_,
        key_retriever_callback_, "", options.batch_size, options.parallelism);
