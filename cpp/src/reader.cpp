...
class PackedRecordBatchReader final : public arrow::RecordBatchReader {
  ...
  arrow::Status open() {
    ...
    for (size_t i = 0; i < column_groups_.size(); ++i) {
      ...
      std::unique_ptr<internal::api::ColumnGroupReader> reader;
      chunk_readers_[i] = internal::api::GroupReaderFactory::create(nullptr, cg, fs_, needed_columns_, properties_,
...
class ReaderImpl : public Reader {
  ...
  [[nodiscard]] arrow::Result<std::shared_ptr<arrow::RecordBatchReader>> get_record_batch_reader(
    ...
    auto reader = std::make_shared<PackedRecordBatchReader>(fs_, needed_column_groups_, projected_schema,
                                                            needed_columns_, properties_, key_retriever_callback_);
