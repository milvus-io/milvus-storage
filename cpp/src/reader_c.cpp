...
std::map<std::string, std::string> convert_read_properties(const ::ReadProperties* properties) {
...
ReaderHandle reader_new(FileSystemHandle fs,
  ...
  auto cpp_reader = new Reader(cpp_fs, cpp_manifest, cpp_schema, cpp_needed_columns, cpp_properties);
  auto cpp_reader_handle = static_cast<ReaderHandle>(cpp_reader);  // Fixed: Don't take address of pointer
