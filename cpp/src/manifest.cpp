...
arrow::Status Manifest::serialize(std::ostream& output_stream) const {
  try {
    ...
    // Recoverable: serialize() is a pure function with no shared state to
    // settle (no promise, no singleflight marker), unlike the fail-stop sites
    // whose escaping exception would strand waiters. Report OOM -- the
    // classification chain maps it to MemAllocateFailed -- so one failed
    // allocation on the commit path fails the commit instead of the node.
    return arrow::Status::OutOfMemory("Failed to serialize Manifest: ", e.what());
