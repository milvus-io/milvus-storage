...
arrow::Result<std::shared_ptr<arrow::Table>> CopySelectedRows(const std::shared_ptr<arrow::Table>& table,
  ...
  for (int i = 0; i < table->num_columns(); ++i) {
    arrow::Datum input(table->column(i));
    // arrow::Datum indices_datum(index_array);

    ARROW_ASSIGN_OR_RAISE(auto datum, arrow::compute::Take(input, index_array, options, &context));
