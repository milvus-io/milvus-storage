...
class TextColumnWriterImpl : public TextColumnWriter {
  ...
  TextColumnWriterImpl(std::shared_ptr<arrow::fs::FileSystem> fs, const TextColumnConfig& config)
      : fs_(std::move(fs)),
        ...
        pending_bytes_(0) {
    // generate UUID string for the first LOB file
    current_file_id_str_ = GenerateUUIDString();
  }
  ...
  arrow::Result<std::vector<uint8_t>> WriteText(const std::string& text) override {
    ...
    auto ref = EncodeLOBReference(current_file_id_str_, row_offset);
  ...
  arrow::Result<std::vector<std::vector<uint8_t>>> WriteBatch(const std::vector<std::string>& texts) override {
    ...
    for (const auto& text : texts) {
      if (text.size() < config_.inline_threshold) {
        ...
        results.push_back(EncodeLOBReference(current_file_id_str_, row_offset));
        stats_.lob_texts++;
  ...
  arrow::Status EnsureVortexWriter() {
    ...
    // build file path
    auto file_path = BuildLOBFilePath(config_.lob_base_path, current_file_id_str_);
  ...
  arrow::Status RollToNewFile() {
    ...
    // generate new file UUID string and reset counters
    current_file_id_str_ = GenerateUUIDString();
    current_file_rows_ = 0;
    current_file_bytes_ = 0;
  ...
  std::string current_file_id_str_;  // UUID string (36 chars)
