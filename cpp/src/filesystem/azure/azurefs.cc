...
std::optional<milvus_storage::ExtendStatusCode> ClassifyAzureError(int http_status,
  ...
  switch (http_status) {
    ...
    case 404:  // Not Found
      ...
      // Gen2 code. Missing either spelling sends an operator's problem to the
      // caller as a missing object.
      if (error_code == "ContainerNotFound" || error_code == "FilesystemNotFound" ||
          reason_phrase == "The specified container does not exist." ||
          reason_phrase == "The specified filesystem does not exist.") {
        return milvus_storage::ExtendStatusCode::AwsErrorBucketNotFound;
      }
      // The blob is gone. Missing, not Permanent: this layer will not say
      // whether that is a GC race or lost data -- the consumer re-reads its
    ...
    case 503:  // Service Unavailable
      // Our own marker first: the SAS policy uses a 503 to carry a transport
      // failure, because HTTP cannot say "the connection dropped".
...
Status ExceptionToStatus(const Azure::Core::RequestFailedException& exception, PrefixArgs&&... prefix_args) {
  ...
  auto code = internal::ClassifyAzureError(static_cast<int>(exception.StatusCode), exception.ErrorCode,
                                           transport_failure, exception.ReasonPhrase);
  if (!code.has_value()) {
    return Status::IOError(std::forward<PrefixArgs>(prefix_args)..., " Azure Error: [", exception.ErrorCode, "] ",
                           exception.what());
  }
...
class ObjectInputFile final : public io::RandomAccessFile {
  ...
  Status EnsureProperties(bool need_metadata) {
    ...
    try {
      ...
      // AwsErrorBucketNotFound (Config), the same split S3 makes between
      // NO_SUCH_KEY and NO_SUCH_BUCKET. This is the read path, so it is where
      // that difference reaches milvus; the delete/GetFileInfo 404 handlers
      // stay as they are on purpose, because absence satisfies a delete and is
      // a value, not an error, for GetFileInfo.
...
Status CreateEmptyBlockBlob(const Blobs::BlockBlobClient& block_blob_client,
  ...
  try {
    ...
    return ExceptionToStatus(exception, "UploadFrom failed for '", block_blob_client.GetUrl(),
...
Status CreateEmptyBlockBlobConditional(Blobs::BlockBlobClient& block_blob_client,
  ...
  try {
    ...
    return ExceptionToStatus(exception, "Conditional upload failed for '", block_blob_client.GetUrl(),
...
Status CommitBlockList(std::shared_ptr<Storage::Blobs::BlockBlobClient> block_blob_client,
  ...
  try {
    ...
    return ExceptionToStatus(exception, "CommitBlockList failed for '", block_blob_client->GetUrl(),
...
Status StageBlock(Blobs::BlockBlobClient* block_blob_client,
  ...
  try {
    ...
    return ExceptionToStatus(exception, "StageBlock failed for '", block_blob_client->GetUrl(), "' new_block_id: '", id,
...
class ObjectAppendStream final : public io::OutputStream {
  ...
  Status AppendBlock(const void* data, int64_t nbytes, std::shared_ptr<Buffer> owned_buffer = nullptr) {
    ...
    if (background_writes_) {
      ...
      auto deferred = [owned_buffer, block_id, block_blob_client = block_blob_client_, state = upload_state_,
                       metrics = metrics_]() mutable -> Status {
  ...
  static void HandleUploadOutcome(const std::shared_ptr<UploadState>& state, const Status& status) {
...
Result<HNSSupport> CheckIfHierarchicalNamespaceIsEnabled(const DataLake::DataLakeFileSystemClient& adlfs_client,
  ...
  try {
    ...
    return arrow::Status::OutOfMemory("Out of memory while checking Hierarchical Namespace support");
...
Result<FileInfo> GetContainerPropsAsFileInfo(const AzureLocation& location, const ContainerClient& container_client) {
  ...
  try {
    ...
    return ExceptionToStatus(exception, "GetProperties for '", container_client.GetUrl(), "' failed.");
  }
}
...
class LeaseGuard {
  ...
  ARROW_NOINLINE void Forget() {
    ...
    while (latest < break_or_expires_at_ &&
...
#ifndef NDEBUG
    int64_t remaining_time_ms = std::chrono::duration_cast<std::chrono::milliseconds>(remaining_time).count();
    LOG_STORAGE_WARNING_ << "LeaseGuard::WaitUntilLatestKnownExpiryTime for " << remaining_time_ms << "ms...";
#endif
...
class AzureFileSystem::Impl {
  ...
  Status CreateDirTemplate(const ContainerClient& container_client,
    ...
    if (recursive) {
      ...
      ARROW_ASSIGN_OR_RAISE(auto container_info, GetContainerPropsAsFileInfo(container, container_client));
  ...
  Status EnsureEmptyDirExists(const Blobs::BlobContainerClient& container_client,
    ...
    try {
      ...
      return ExceptionToStatus(exception, operation_name, " failed to ensure empty directory marker '", location.path,
                               "' exists in container: ", container_client.GetUrl());
    }
  }
  ...
  Status DeleteDirContentsOnContainer(const Blobs::BlobContainerClient& container_client,
    ...
    try {
      ...
      for (; list_response.HasPage(); list_response.MoveToNextPage()) {
        ...
        // One category or nothing -- the same rule as the S3 multi-delete, and
        // it lives in one place for the same reason: two backends must not
        // disagree about the same mixed batch.
        //
        // A blob that comes back Deleted=false WITHOUT throwing counts as an
        // unclassified member. That path is why the previous precedence ladder
        // was wrong here: it only ranked thrown exceptions, so Deleted=false
        // beside a 503 answered "Transient" -- exactly the outcome the ladder
        // existed to prevent, in the very place it was not applied. Under this
        // rule an unclassified member is a disagreement by construction, and
        // there is no path that skips it.
        std::optional<Status> representative;
        std::optional<milvus_storage::ErrorCategory> shared_category;
        bool mixed = false;
        auto observe = [&](std::optional<Status> failure) {
          auto category = milvus_storage::ErrorCategory::Unknown;
          if (failure.has_value()) {
            if (auto detail = milvus_storage::ExtendStatusDetail::UnwrapStatus(*failure)) {
              category = detail->category();
            }
          }
          if (!shared_category.has_value()) {
            shared_category = category;
            representative = std::move(failure);
        ...
        for (auto& [blob_name_view, deferred_response] : deferred_responses) {
          ...
          try {
            ...
            success = false;
            observe(ExceptionToStatus(exception, "Failed to delete blob: ", blob_name_view, ": "));
            failed_blob_names.emplace_back(blob_name_view);
            continue;
          }
          if (!success) {
            // Reported failed with no exception, so it carries no reason at
            // all. Unclassified, and counted.
            observe(std::nullopt);
            failed_blob_names.emplace_back(blob_name_view);
          }
        }
        if (!failed_blob_names.empty()) {
          ...
          if (failed_blob_names.size() == 1) {
            message_stream << "Failed to delete a blob: " << failed_blob_names[0] << ": " << container_client.GetUrl();
          } else {
            message_stream << "Failed to delete blobs: [" << arrow::internal::JoinStrings(failed_blob_names, ", ")
                           << "]: " << container_client.GetUrl();
          }
          auto message = message_stream.str();
          // A lone failure is returned as itself. Summarizing one thing loses
          // whatever the Status carried beyond the code this loop extracted --
          // arrow's errno detail among it, which callers use to recognize a
          // missing object.
          if (!mixed && representative.has_value()) {
            // WithMessage keeps the code and the detail.
            return representative->WithMessage(message + " " + representative->message());
          }
  ...
  Status DeleteDirOnFileSystem(const DataLake::DataLakeFileSystemClient& adlfs_client,
    ...
    try {
      ...
      if (exception.ErrorCode == "FilesystemNotFound" || exception.ErrorCode == "PathNotFound") {
        if (require_dir_to_exist) {
          return PathNotFound(location);
        }
        return Status::OK();
  ...
  Status RenameContainer(const AzureLocation& src, const AzureLocation& dest) {
    ...
    if (!dest_exists) {
      ...
      try {
        ...
        return ExceptionToStatus(exception, "Unable to replace empty container: '", dest.all,
