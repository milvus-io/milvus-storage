...
struct NativeS3Operations::Path {
  std::string bucket;
  std::string key;
  static arrow::Result<Path> Parse(const std::string& path) {
    if (arrow::fs::internal::IsLikelyUri(path) || (!path.empty() && path.front() == '/')) {
      return arrow::Status::Invalid("Expected bucket/key path: ", path);
    }
    if (path.find('\0') != std::string::npos)
      return arrow::Status::Invalid("NUL in S3 path");
    ...
    for (const auto& component : arrow::fs::internal::SplitAbstractPath(clean)) {
      if (component == "." || component == "..")
        return arrow::Status::Invalid("Dot components in S3 path");
    }
...
NativeS3Operations::NativeS3Operations(std::shared_ptr<NativeS3Transport> transport, arrow::io::IOContext io)
    : transport_(std::move(transport)), io_(std::move(io)) {}

Future<NativeS3ObjectMetadata> NativeS3Operations::ReadMetadataAsync(const std::string& path,
...
Future<FileInfo> NativeS3Operations::GetFileInfoAsync(const std::string& path) {
...
arrow::fs::FileInfoGenerator NativeS3Operations::GetFileInfoGenerator(const FileSelector& selector) {
  struct Listing {
    std::shared_ptr<NativeS3Operations> fs;
...
Result<std::shared_ptr<NativeS3Operations>> MakeNativeS3Operations(const arrow::io::IOContext& io_context,
                                                                   std::shared_ptr<NativeS3Transport> transport) {
  if (!transport)
    return Status::Invalid("Native S3 requires the filesystem transport");
  return std::make_shared<NativeS3Operations>(std::move(transport), io_context);
}
}  // namespace milvus_storage
#endif
