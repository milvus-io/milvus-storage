...
arrow::Status SetObjectMetadata(const std::shared_ptr<const arrow::KeyValueMetadata>& metadata, ObjectRequest* req) {
  ...
  for (size_t i = 0; i < keys.size(); ++i) {
    // ignore private key
    if (keys[i] == MultiPartUploadSizeKey) {
      continue;
    }
...
class CustomOutputStream final : public arrow::io::OutputStream {
  ...
  CustomOutputStream(std::shared_ptr<S3ClientHolder> holder,
    ...
    if (metadata_) {
      if (int64_t keyidx = metadata_->FindKey(MultiPartUploadSizeKey); keyidx != -1) {
        auto part_size_str = metadata_->value(keyidx);
        ...
        if (ec == std::errc() && ptr == part_size_str.data() + part_size_str.size()) {
          part_upload_size_ = part_size_result;
        } else {
