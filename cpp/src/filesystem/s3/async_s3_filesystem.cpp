...
class AsyncS3FileSystem final : public NativeS3Operations, public std::enable_shared_from_this<AsyncS3FileSystem> {
  public:
  AsyncS3FileSystem(std::shared_ptr<NativeS3Transport> transport, arrow::io::IOContext io)
...
Result<std::shared_ptr<NativeS3Operations>> MakeNativeS3Operations(const arrow::io::IOContext& io_context,
  ...
  return std::make_shared<AsyncS3FileSystem>(std::move(transport), io_context);
}
}  // namespace milvus_storage
#endif
