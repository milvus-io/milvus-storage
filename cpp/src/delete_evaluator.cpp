...
static arrow::Result<std::string> ResolveTimestampFieldNameByFieldId(const std::shared_ptr<arrow::Schema>& schema) {
  ARROW_ASSIGN_OR_RAISE(auto ts_index, FindFieldIndexByFieldId(schema, kMilvusTimestampFieldId));
  if (ts_index < 0) {
...
class DeleteEvaluator {
    ...
    for (const auto& delta_log : manifest_->deltaLogs()) {
      ...
      switch (delta_log.type) {
        ...
        case DeltaLogType::POSITIONAL:
          return arrow::Status::NotImplemented("Unsupported delta log: Positional type for alive reader: ",
                                               delta_log.path);
        case DeltaLogType::EQUALITY:
          return arrow::Status::NotImplemented("Unsupported delta log: Equality type for alive reader: ",
                                               delta_log.path);
        default:
          return arrow::Status::Invalid(
  ...
  arrow::Status EnsurePrimaryKeyDeleteSupport() {
    ...
    if (!options_.pk_field_id.has_value()) {
      return arrow::Status::Invalid(
          "PRIMARY_KEY delta logs require AliveReadOptions.pk_field_id: milvus-storage has no primary-key concept, so "
  ...
  arrow::Status EnsureRowTimestampField() const {
    ARROW_ASSIGN_OR_RAISE(auto timestamp_field_name, ResolveTimestampFieldNameByFieldId(schema_));
  ...
  arrow::Status LoadPredicateDeltaLog(const DeltaLog& delta_log,
    ...
    while (true) {
      ...
      ARROW_RETURN_NOT_OK(
          LoadPredicateDeletes(*delete_ts_array, *predicate_sql_array, timestamp_field_name, delta_log.path));
    }
    return reader->Close();
  }

  arrow::Status LoadPredicateDeletes(const arrow::Int64Array& delete_ts_array,
    ...
    for (int64_t i = 0; i < delete_ts_array.length(); ++i) {
      ...
      ARROW_ASSIGN_OR_RAISE(auto delete_expr,
