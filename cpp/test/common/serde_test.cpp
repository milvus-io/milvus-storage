...
TEST_F(SerdeTest, TestColumnOffsetSerde) {
  std::vector<std::vector<int>> column_offsets = {{0, 1, 2}, {3, 4}, {5, 6, 7, 8}};

  std::string serialized = PackedMetaSerde::SerializeColumnOffsets(column_offsets);
  EXPECT_EQ(serialized, "0,1,2;3,4;5,6,7,8");

  std::vector<std::vector<int>> deserialized = PackedMetaSerde::DeserializeColumnOffsets(serialized);
  EXPECT_EQ(deserialized, column_offsets);

  // Test case: Empty input
  std::vector<std::vector<int>> empty_offsets = {};
  ...
  std::vector<std::vector<int>> single_group = {{1, 2, 3}};
  ...
  std::vector<std::vector<int>> single_column_groups = {{0}, {1}, {2}};
