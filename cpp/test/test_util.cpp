...
void InitTestProperties(api::Properties& properties, std::string address, std::string root_path) {
  if (GetEnvVar(ENV_VAR_STORAGE_TYPE).ValueOr("") == "local" || GetEnvVar(ENV_VAR_STORAGE_TYPE).ValueOr("").empty()) {
    ...
    api::SetValue(properties, PROPERTY_FS_ADDRESS,
                  GetEnvVar(ENV_VAR_ADDRESS).ValueOr("").empty() ? "http://localhost:9000"
                                                                 : GetEnvVar(ENV_VAR_ADDRESS).ValueOr("").c_str());

    api::SetValue(properties, PROPERTY_FS_BUCKET_NAME,
                  GetEnvVar(ENV_VAR_BUCKET_NAME).ValueOr("").empty()
                      ? "test-bucket"
                      : GetEnvVar(ENV_VAR_BUCKET_NAME).ValueOr("").c_str());

    api::SetValue(properties, PROPERTY_FS_ACCESS_KEY_ID,
                  GetEnvVar(ENV_VAR_ACCESS_KEY_ID).ValueOr("").empty()
                      ? "minioadmin"
                      : GetEnvVar(ENV_VAR_ACCESS_KEY_ID).ValueOr("").c_str());

    api::SetValue(properties, PROPERTY_FS_ACCESS_KEY_VALUE,
                  GetEnvVar(ENV_VAR_ACCESS_KEY_VALUE).ValueOr("").empty()
                      ? "minioadmin"
                      : GetEnvVar(ENV_VAR_ACCESS_KEY_VALUE).ValueOr("").c_str());

    api::SetValue(properties, PROPERTY_FS_REGION,
                  GetEnvVar(ENV_VAR_REGION).ValueOr("").empty() ? "" : GetEnvVar(ENV_VAR_REGION).ValueOr("").c_str());

    api::SetValue(
        properties, PROPERTY_FS_REGION,
        GetEnvVar(ENV_VAR_ROOT_PATH).ValueOr("").empty() ? "files" : GetEnvVar(ENV_VAR_ROOT_PATH).ValueOr("").c_str());
  }
}
