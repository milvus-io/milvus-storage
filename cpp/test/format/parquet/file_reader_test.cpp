...
TEST_F(FileReaderTest, ReadAllColumnsWithEnoughMemory) {
  ...
  SetupOneFile();
  // read all row groups
  ASSERT_AND_ASSIGN(auto fr, FileRowGroupReader::Make(fs_, one_file_path_, schema_, reader_memory_));
...
TEST_F(FileReaderTest, ReadAllColumnsWithFewerMemory) {
  ...
  ASSERT_AND_ASSIGN(auto fr, FileRowGroupReader::Make(fs_, one_file_path_, schema_, fewer_memory));
  auto row_group_sizes = fr->file_metadata()->GetRowGroupMetadataVector();
  ASSERT_STATUS_OK(fr->SetRowGroupOffsetAndCount(0, row_group_sizes.size()));
...
TEST_F(FileReaderTest, SchemaEvolutionMoreColumns) {
  ...
  ASSERT_AND_ASSIGN(auto fr, FileRowGroupReader::Make(fs_, one_file_path_, new_schema, reader_memory_));
  auto row_group_sizes = fr->file_metadata()->GetRowGroupMetadataVector();
  ASSERT_STATUS_OK(fr->SetRowGroupOffsetAndCount(0, row_group_sizes.size()));
...
TEST_F(FileReaderTest, ReadWithoutSchema) {
  ...
  ASSERT_AND_ASSIGN(auto fr, FileRowGroupReader::Make(fs_, one_file_path_, reader_memory_));
  auto row_group_sizes = fr->file_metadata()->GetRowGroupMetadataVector();
  ASSERT_STATUS_OK(fr->SetRowGroupOffsetAndCount(0, row_group_sizes.size()));
...
TEST_F(FileReaderTest, ReadWithNotEnoughMemory) {
  SetupOneFile();
  ASSERT_AND_ASSIGN(auto fr, FileRowGroupReader::Make(fs_, one_file_path_, schema_, 1024));
  auto row_group_sizes = fr->file_metadata()->GetRowGroupMetadataVector();
  ASSERT_STATUS_OK(fr->SetRowGroupOffsetAndCount(0, row_group_sizes.size()));
...
TEST_F(FileReaderTest, FileNotExists) {
  ...
  // FileRowGroupReader(fs_, non_existent_path, schema_, reader_memory_);
  ASSERT_FALSE(FileRowGroupReader::Make(fs_, non_existent_path, schema_, reader_memory_).ok());
}

TEST_F(FileReaderTest, InvalidBufferSize) {
  SetupOneFile();
  // Test with negative buffer size
  ASSERT_AND_ASSIGN(auto fr, FileRowGroupReader::Make(fs_, one_file_path_, schema_, -1));
  auto row_group_sizes = fr->file_metadata()->GetRowGroupMetadataVector();
...
TEST_F(FileReaderTest, ZeroBufferSize) {
  ...
  ASSERT_AND_ASSIGN(auto fr, FileRowGroupReader::Make(fs_, one_file_path_, schema_, 0));
  auto row_group_sizes = fr->file_metadata()->GetRowGroupMetadataVector();
  ASSERT_STATUS_OK(fr->SetRowGroupOffsetAndCount(0, 1));
...
TEST_F(FileReaderTest, SchemaEvolutionWithInvalidFieldID) {
  ...
  ASSERT_FALSE(FileRowGroupReader::Make(fs_, one_file_path_, invalid_schema, reader_memory_).ok());
}
...
TEST_F(FileReaderTest, SchemaEvolutionWithMissingFieldIDMetadata) {
  ...
  ASSERT_FALSE(FileRowGroupReader::Make(fs_, one_file_path_, schema_without_id, reader_memory_).ok());
}
...
TEST_F(FileReaderTest, MemoryPressureWithLargeRowGroups) {
  ...
  ASSERT_AND_ASSIGN(auto fr, FileRowGroupReader::Make(fs_, one_file_path_, schema_, tiny_memory));
  auto row_group_sizes = fr->file_metadata()->GetRowGroupMetadataVector();
  ASSERT_STATUS_OK(fr->SetRowGroupOffsetAndCount(0, row_group_sizes.size()));
...
TEST_F(FileReaderTest, SchemaEvolutionWithDuplicateFieldIDs) {
  ...
  ASSERT_AND_ASSIGN(auto fr, FileRowGroupReader::Make(fs_, one_file_path_, duplicate_schema, reader_memory_));
  auto row_group_sizes = fr->file_metadata()->GetRowGroupMetadataVector();
  ASSERT_STATUS_OK(fr->SetRowGroupOffsetAndCount(0, 1));
...
TEST_F(FileReaderTest, VeryLargeBufferSize) {
  ...
  ASSERT_AND_ASSIGN(auto fr, FileRowGroupReader::Make(fs_, one_file_path_, schema_, huge_memory));
  auto row_group_sizes = fr->file_metadata()->GetRowGroupMetadataVector();
  ASSERT_STATUS_OK(fr->SetRowGroupOffsetAndCount(0, row_group_sizes.size()));
...
TEST_F(FileReaderTest, NullSchemaPointer) {
  ...
  ASSERT_AND_ASSIGN(auto fr, FileRowGroupReader::Make(fs_, one_file_path_, nullptr, reader_memory_));
  auto row_group_sizes = fr->file_metadata()->GetRowGroupMetadataVector();
  ASSERT_STATUS_OK(fr->SetRowGroupOffsetAndCount(0, 1));
