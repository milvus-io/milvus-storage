...
TEST(VortexErrorTest, StreamingReaderTranslatesReadNextBridgeError) {
  auto inner = std::make_shared<FailingRecordBatchReader>(
      arrow::Status::IOError(fmt::format("__LOON_FFI_ERRCODE__={}; readat failed", LOON_TRANSIENT_NETWORK)));
...
TEST(VortexErrorTest, StreamingReaderTranslatesReadNextFileNotFound) {
  auto inner = std::make_shared<FailingRecordBatchReader>(
      arrow::Status::IOError(fmt::format("__LOON_FFI_ERRCODE__={}; file not found", LOON_FILE_NOT_FOUND)));
  auto reader = vortex::internal::WrapVortexRecordBatchReader(std::move(inner));
...
TEST(VortexErrorTest, StreamingReaderTranslatesCloseBridgeError) {
  auto inner = std::make_shared<FailingRecordBatchReader>(
      arrow::Status::IOError(fmt::format("__LOON_FFI_ERRCODE__={}; close failed", LOON_TRANSIENT_TIMEOUT)));
  auto reader = vortex::internal::WrapVortexRecordBatchReader(std::move(inner));
...
TEST(VortexErrorTest, MapsBridgeErrorCodesToStatusDetails) {
  auto file_not_found_status = MakeBridgeErrorStatus(
      "Failed to read vortex file", fmt::format("__LOON_FFI_ERRCODE__={}; file not found", LOON_FILE_NOT_FOUND));
  ...
  EXPECT_EQ(file_not_found_status.ToString().find("__LOON_FFI_ERRCODE__"), std::string::npos);

  auto aws_not_found_status = MakeBridgeErrorStatus(
      "Failed to read vortex file", fmt::format("__LOON_FFI_ERRCODE__={}; object not found", LOON_AWS_ERROR_NOT_FOUND));
  ...
  auto timeout_status = MakeBridgeErrorStatus(
      "Failed to read vortex file", fmt::format("__LOON_FFI_ERRCODE__={}; read failed", LOON_TRANSIENT_TIMEOUT));
  ...
  auto upload_status = MakeBridgeErrorStatus(
      "Failed to close Vortex file", fmt::format("outer __LOON_FFI_ERRCODE__={}; Failed to close ObjectStoreWriterCpp",
  ...
  EXPECT_EQ(upload_status.ToString().find("__LOON_FFI_ERRCODE__"), std::string::npos);

  auto network_status = MakeBridgeErrorStatus(
  ...
  EXPECT_EQ(network_status.ToString().find("__LOON_FFI_ERRCODE__"), std::string::npos);

  auto txn_status = MakeBridgeErrorStatus(
      "Failed to write Vortex file", fmt::format("__LOON_FFI_ERRCODE__={}; commit failed", LOON_TXN_EXHAUSTED_RETRY));
