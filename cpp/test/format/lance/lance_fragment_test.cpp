...
TEST_F(LanceBasicTest, TestBasic) {
  ...
  std::shared_ptr<api::ColumnGroup> column_group = std::make_shared<api::ColumnGroup>();
  column_group->format = LOON_FORMAT_LANCE_FRAGMENT;
  column_group->columns = {"id", "name", "vector"};
  column_group->files.emplace_back(api::ColumnGroupFile{
      .path = base_path_ + "/lance_vector_dataset",
      .start_index = -1,
      .end_index = -1,
  });
  ...
  {
    LanceFragmentWriter writer(column_group, schema_, properties_);
    for (int i = 0; i < num_of_batches; i++) {
      ASSERT_STATUS_OK(writer.Write(test_batch_));
    }
    ASSERT_STATUS_OK(writer.Close());
    ASSERT_EQ(writer.written_rows(), test_batch_->num_rows() * num_of_batches);
  }
  ...
  auto verify_reader = [&]() {
    ...
    for (const auto& fragment_id : fragment_ids) {
      LanceFragmentReader reader(read_dataset, fragment_id, schema_, properties_);
...
TEST_F(LanceBasicTest, TestRead) {
  ...
  ASSERT_STATUS_OK(writer.Close());
  ASSERT_EQ(writer.written_rows(), large_batch->num_rows());
  ...
  LanceFragmentReader reader(read_dataset, fragment_ids[0], schema_, properties_);
  ASSERT_STATUS_OK(reader.open());
  ASSERT_AND_ASSIGN(auto rgs, reader.get_row_group_infos());
  ASSERT_FALSE(rgs.empty());
