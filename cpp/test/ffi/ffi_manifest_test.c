...
static void test_add_delta_log(void) {
  ...
  // Add delta log
  rc = loon_transaction_add_delta_log(tranhandle, "delta_log_path_1.log", 100, LOON_DELTA_LOG_TYPE_PRIMARY_KEY);
  ck_assert_msg(loon_ffi_is_success(&rc), "%s", loon_ffi_get_errmsg(&rc));
...
static void test_transaction_error_handling(void) {
  ...
  rc = loon_transaction_add_delta_log(0, "path", 100, LOON_DELTA_LOG_TYPE_PRIMARY_KEY);
  ck_assert(!loon_ffi_is_success(&rc));
  loon_ffi_free_result(&rc);

  rc = loon_transaction_add_delta_log((LoonTransactionHandle)1, NULL, 100, LOON_DELTA_LOG_TYPE_PRIMARY_KEY);
  ck_assert(!loon_ffi_is_success(&rc));
  loon_ffi_free_result(&rc);

  rc = loon_transaction_add_delta_log((LoonTransactionHandle)1, "path", 100, 999);
  ck_assert(!loon_ffi_is_success(&rc));
  loon_ffi_free_result(&rc);
