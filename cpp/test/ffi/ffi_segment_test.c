...
static void test_segment_writer_reader_basic(void) {
  ...
  struct ArrowArrayStream parent_destroyed_stream;
  ...
  memset(&parent_destroyed_stream, 0, sizeof(parent_destroyed_stream));
  rc = loon_segment_reader_get_stream(reader, &parent_destroyed_stream);
  ck_assert_msg(loon_ffi_is_success(&rc), "%s", loon_ffi_get_errmsg(&rc));
  ...
  int arrow_rc = parent_destroyed_stream.get_next(&parent_destroyed_stream, &out_array);
  ck_assert(arrow_rc != 0);
  ck_assert(out_array.release == NULL);
  ck_assert(parent_destroyed_stream.get_last_error(&parent_destroyed_stream) != NULL);
  ck_assert_msg(strstr(parent_destroyed_stream.get_last_error(&parent_destroyed_stream), "after parent close") != NULL,
                "unexpected stream error: %s", parent_destroyed_stream.get_last_error(&parent_destroyed_stream));
  parent_destroyed_stream.release(&parent_destroyed_stream);
...
static void test_segment_error_handling(void) {
  ...
  const char* null_column[] = {NULL};
  rc = loon_segment_reader_open("segment", -1, &dummy_schema, NULL, 1, &reader_config, &dummy_properties, &reader);
  ck_assert_int_eq(rc.err_code, loon_errcode_invalid_args);
  loon_ffi_free_result(&rc);

  rc = loon_segment_reader_open("segment", -1, &dummy_schema, null_column, 1, &reader_config, &dummy_properties,
                                &reader);
  ck_assert_int_eq(rc.err_code, loon_errcode_invalid_args);
  loon_ffi_free_result(&rc);
  ...
  rc = loon_segment_reader_get_chunk_reader((LoonSegmentReaderHandle)1, 0, null_column, 1, &chunk_reader);
  ck_assert_int_eq(rc.err_code, loon_errcode_invalid_args);
  loon_ffi_free_result(&rc);
