...
static int row_mask_bit_is_set(const LoonRowMask* bitset, int64_t index) {
...
static void test_alive_reader_primary_key_delete_mask(void) {
  ...
  LoonFFIResult rc;
  LoonProperties rp;
  LoonMaskedReaderHandle masked_reader;
  ...
  // supplied explicitly via LoonMaskedReadOptions.pk_field_id below.
  ...
  rc = create_test_reader_pp(&rp);
  ck_assert_msg(loon_ffi_is_success(&rc), "%s", loon_ffi_get_errmsg(&rc));
  rc = loon_masked_reader_new(&manifest, schema, needed_columns, 3, &rp, &masked_reader, &options,
                              NULL /*key_retriever*/);
  ck_assert_msg(loon_ffi_is_success(&rc), "%s", loon_ffi_get_errmsg(&rc));

  struct ArrowArray* out_array = NULL;
  struct ArrowSchema* out_schema = NULL;
  LoonRowMask row_mask;
  rc = loon_masked_reader_next(masked_reader, &out_array, &out_schema, &row_mask);
  ck_assert_msg(loon_ffi_is_success(&rc), "%s", loon_ffi_get_errmsg(&rc));
  verify_alive_pk_record_batch(out_array, out_schema);
  ck_assert(row_mask.data != NULL);
  ck_assert_int_eq(row_mask.num_bits, out_array->length);
  ck_assert_int_eq(row_mask_bit_is_set(&row_mask, 0), 1);
  ck_assert_int_eq(row_mask_bit_is_set(&row_mask, 1), 0);
  ck_assert_int_eq(row_mask_bit_is_set(&row_mask, 2), 1);
  ck_assert_int_eq(row_mask_bit_is_set(&row_mask, 3), 1);
  ck_assert_int_eq(row_mask_bit_is_set(&row_mask, 4), 1);

  loon_row_mask_free(&row_mask);
  ...
  ck_assert(out_schema == NULL);
  ck_assert(row_mask.data == NULL);
  ck_assert_int_eq(row_mask.num_bits, 0);

  loon_masked_reader_destroy(masked_reader);
...
static void test_alive_reader_predicate_delete_mask(void) {
  ...
  rc = create_test_reader_pp(&rp);
  ck_assert_msg(loon_ffi_is_success(&rc), "%s", loon_ffi_get_errmsg(&rc));
  rc = loon_masked_reader_new(&manifest, schema, needed_columns, 1, &rp, &masked_reader, &options,
                              NULL /*key_retriever*/);
  ck_assert_msg(loon_ffi_is_success(&rc), "%s", loon_ffi_get_errmsg(&rc));
  ...
  ck_assert_int_eq(row_mask_bit_is_set(&row_mask, 0), 1);
  ck_assert_int_eq(row_mask_bit_is_set(&row_mask, 1), 0);
  ck_assert_int_eq(row_mask_bit_is_set(&row_mask, 2), 0);
  ck_assert_int_eq(row_mask_bit_is_set(&row_mask, 3), 1);
  ck_assert_int_eq(row_mask_bit_is_set(&row_mask, 4), 1);
...
static void test_alive_reader_noop_mask(void) {
  ...
  rc = create_test_reader_pp(&rp);
  ck_assert_msg(loon_ffi_is_success(&rc), "%s", loon_ffi_get_errmsg(&rc));
  rc = loon_masked_reader_new(&manifest, schema, needed_columns, 3, &rp, &masked_reader, NULL, NULL /*key_retriever*/);
  ck_assert_msg(loon_ffi_is_success(&rc), "%s", loon_ffi_get_errmsg(&rc));
  ...
  while (true) {
    ...
    ck_assert(row_mask.data != NULL);
    ck_assert_int_eq(row_mask.num_bits, out_array->length);
    ck_assert_int_eq(row_mask.bit_offset, 0);
    for (int64_t i = 0; i < row_mask.num_bits; ++i) {
      ck_assert_msg(row_mask_bit_is_set(&row_mask, i), "row mask bit at row %" PRId64 " should be true", i);
    }
