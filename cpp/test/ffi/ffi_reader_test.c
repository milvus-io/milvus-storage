...
static void test_reader_error_handling(void) {
  ...
  // Projection arrays are public C inputs. A non-zero count requires both the
  // top-level pointer and every entry to be non-NULL.
  {
    LoonColumnGroups column_groups = {0};
    struct ArrowSchema schema = {0};
    LoonProperties properties = {0};
    const char* null_column[] = {NULL};

    rc = loon_reader_new(&column_groups, &schema, NULL, 1, &properties, &reader_handle);
    ck_assert_int_eq(rc.err_code, loon_errcode_invalid_args);
    loon_ffi_free_result(&rc);

    rc = loon_get_chunk_reader((LoonReaderHandle)1, 0, null_column, 1, &chunk_reader_handle);
    ck_assert_int_eq(rc.err_code, loon_errcode_invalid_args);
    loon_ffi_free_result(&rc);

    int64_t row_index[] = {0};
    rc = loon_take((LoonReaderHandle)1, row_index, 1, 1, NULL, 1, &arrays, &num_arrays, NULL);
    ck_assert_int_eq(rc.err_code, loon_errcode_invalid_args);
    loon_ffi_free_result(&rc);
  }
