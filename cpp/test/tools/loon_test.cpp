...
TEST_F(LoonTest, CreateAndReadIceberg) {
  ...
  ASSERT_AND_ASSIGN(auto input_file, fs_->OpenInputFile(manifest_path));
  ASSERT_AND_ASSIGN(auto file_size, input_file->GetSize());
  ASSERT_AND_ASSIGN(auto buf, input_file->Read(file_size));
  ...
  ASSERT_STATUS_OK(manifest->deserialize(in));
