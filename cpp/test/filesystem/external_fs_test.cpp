...
TEST_F(ExternalFilesystemTest, AzureCredentialBrokerConfigValidation) {
  ...
  auto AssertBrokerConfigError = [](const arrow::Status& status, const char* what) {
    ...
    EXPECT_FALSE((CategoryForExtendStatusCode(detail->code()) == ErrorCategory::Transient)) << what;
  };
...
TEST_F(ExternalFilesystemTest, UnparseableUriIsClassifiedAsConfigNotGuessedAsUser) {
  ...
  for (const auto& c : cases) {
    ...
    EXPECT_FALSE((CategoryForExtendStatusCode(*code) == ErrorCategory::Transient)) << c.what;
