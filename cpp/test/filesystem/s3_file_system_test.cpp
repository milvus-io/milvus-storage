...
TEST_F(S3UnitTest, TestErrorToStatusPermanentVsTransient) {
  ...
  // An error nothing recognized stays unclassified. It used to be tagged
  // permanent on the strength of ShouldRetry() == false, which is the SDK's
  // retry policy and not a statement about the condition -- the two guards that
  // rule had accumulated (UNKNOWN types, connect errors) were each a backend it
...
// every arm and were tagged from the SDK's retry verdict -- a local allocation
...
TEST_F(S3UnitTest, CoreErrorsAreClassifiedBeforeTheS3Cast) {
  {
    ...
    auto status = fs::internal::ErrorToStatus("test", error);
    ASSERT_STATUS_NOT_OK(status);
    // Arrow's own code is where the allocation failure survives; nothing here
    // translates it into a consumer's vocabulary any more.
  ...
  for (auto core : {Aws::Client::CoreErrors::UNRECOGNIZED_CLIENT, Aws::Client::CoreErrors::INVALID_SIGNATURE}) {
    ...
    EXPECT_NE(detail->category(), ErrorCategory::Transient);
  }
}

TEST_F(S3UnitTest, Conflict409IsNotForPermanentBucketStates) {
  // A 409 with no recognized-permanent name is a race: Conflict -- re-read
  // state and re-submit, which is NOT the generic retryable bit. This is what
  // the transaction commit path relies on, so the classification must survive
  // the blocklist below.
  {
    ...
    EXPECT_EQ(detail->category(), ErrorCategory::Conflict);
    EXPECT_NE(detail->category(), ErrorCategory::Transient) << "rebase is not replay";
  }
...
TEST_F(S3UnitTest, TestErrorToStatus) {
  ...
  // Nothing named the cause, but the SDK says it clears. Transient, with no
  // cause claimed -- this used to report Network for a 500-shaped failure.
  {
    Aws::Client::AWSError<Aws::S3::S3Errors> error(
    ...
    AssertRetryableCode(status, ExtendStatusCode::StorageTransientUnspecified);
  }
  ...
  // PRECONDITION_FAILED
  {
    ...
    // Conflict class: recoverable only by a caller that re-reads before
    // re-submitting. Replaying the same conditional write fails identically,
    // so the generic retryable bit is false and the category carries the rest.
    EXPECT_EQ(detail->category(), ErrorCategory::Conflict);
    EXPECT_NE(detail->category(), ErrorCategory::Transient);
  }

  // A 409 whose name says a race: classified.
  {
    Aws::Client::AWSError<Aws::S3::S3Errors> error(
        Aws::S3::S3Errors::UNKNOWN, Aws::Client::RetryableType::NOT_RETRYABLE, "OperationAborted", "conflict");
    error.SetResponseCode(Aws::Http::HttpResponseCode::CONFLICT);
    auto status = fs::internal::ErrorToStatus("prefix", "PutObject", error);
    ASSERT_FALSE(status.ok());
    auto detail = ExtendStatusDetail::UnwrapStatus(status);
  ...
  // A 409 whose name we do not recognize stays unclassified. 409 is overloaded
  // -- BucketNotEmpty and InvalidBucketState are 409s about the bucket, not
  // about a race -- and this used to blocklist those two and call every other
  // shape a Conflict, which is a guess about spellings nobody here has seen.
  {
    Aws::Client::AWSError<Aws::S3::S3Errors> error(Aws::S3::S3Errors::UNKNOWN,
