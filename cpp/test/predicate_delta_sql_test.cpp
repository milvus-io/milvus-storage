...
class PredicateDeltaSqlTest : public ::testing::Test {
  ...
  arrow::Result<std::shared_ptr<api::DeleteEvaluator>> CreateEvaluator(const std::shared_ptr<api::Manifest>& manifest,
                                                                       ...
                                                                       api::MaskedReadOptions options = {}) {
    // milvus-storage requires the caller to declare the row-timestamp field id; the
    // test fixture's DataSchema always tags its timestamp field with field id 1.
    if (!options.row_timestamp_field_id.has_value()) {
...
TEST_F(PredicateDeltaSqlTest, LoadsOnlyManifestPredicateDeltaType) {
  auto schema = DataSchema();
  ASSERT_AND_ASSIGN(auto delta_batch, PredicateDeltaBatch({25}, {"value > 100"}));
  // A predicate-shaped delta file tagged as a non-predicate manifest type must be
  // rejected by type (not silently loaded as predicate), confirming dispatch is by
  // the manifest delta_log.type rather than the delta-file schema.
  ASSERT_AND_ASSIGN(auto positional_manifest,
                    ManifestWithPredicateDelta(schema, delta_batch, api::DeltaLogType::POSITIONAL));
  auto positional_result = CreateEvaluator(positional_manifest, schema);
  ASSERT_FALSE(positional_result.ok());
  EXPECT_NE(positional_result.status().ToString().find("Unsupported delta log"), std::string::npos);
}
