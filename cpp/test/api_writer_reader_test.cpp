...
TEST_P(APIWriterReaderTest, TakeMethodTest) {
  ...
  std::vector<std::vector<int64_t>> test_row_indices = {
      ...
      all_row_indices,
      // take some rows which need reorder
      {90, 40, 30, 41, 50},
      // take with not unique row indices
      {0, 1, 2, 2, 5, 5, 5, 7},
      // take with not unique row indices and need reorder
      {0, 20, 2, 2, 5, 5, 5, 7, 20, 2, 5, 79, 20},
  };
...
TEST_P(APIWriterReaderTest, TakeWithMultiFiles) {
  ...
  // random rows
  int random_count = 50;
  std::vector<int64_t> random_indices(random_count);
  ...
  std::vector<std::vector<int64_t>> test_row_indices = {
      ...
      {900, 400, 30, 41, 500},
      // take with not unique row indices
      {0, 100, 2000, 2000, 500, 500, 588, 7},
      // take with not unique row indices and need reorder
      {0, 20, 500, 500, 5, 5, 7, 20, 200, 5, 790, 20},
      // take random rows
      random_indices,
  };
