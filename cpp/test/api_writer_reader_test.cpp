...
TEST_P(APIWriterReaderTest, TestNullableFields) {
  ...
  auto writer = Writer::create(fs_, base_path_ + "/" + format, nullable_schema, std::move(policy), properties_);
  ASSERT_NE(writer, nullptr);
  ...
  // reader
  auto reader = Reader::create(fs_, manifest, nullable_schema, nullptr, properties_);
  ASSERT_NE(reader, nullptr);
...
TEST_P(APIWriterReaderTest, TestMixedNullableAndNonNullable) {
  ...
  auto writer = Writer::create(fs_, base_path_ + "/" + format, mixed_schema, std::move(policy), properties_);
  ASSERT_NE(writer, nullptr);
  ...
  auto reader1 = Reader::create(fs_, manifest, mixed_schema, nullptr, properties_);
  ...
  auto reader2 = Reader::create(fs_, manifest, evolved_schema, nullptr, properties_);
...
TEST_P(APIWriterReaderTest, TestAllNullFields) {
  ...
  auto writer = Writer::create(fs_, base_path_ + "/" + format, all_nullable_schema, std::move(policy), properties_);
  ASSERT_NE(writer, nullptr);
  ...
  // reader
  auto reader = Reader::create(fs_, manifest, all_nullable_schema, nullptr, properties_);
  ASSERT_NE(reader, nullptr);
  auto batch_reader_result = reader->get_record_batch_reader();
  ASSERT_TRUE(batch_reader_result.ok()) << batch_reader_result.status().ToString();
  auto batch_reader = std::move(batch_reader_result).ValueOrDie();
