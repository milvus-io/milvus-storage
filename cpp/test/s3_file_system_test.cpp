...
TEST_F(S3FsTest, ConditionalWrite) {
  std::string bucket_name = GetEnvVar(ENV_VAR_BUCKET_NAME).ValueOr("");
  std::string file_from = bucket_name + "/test_conditional_rename_from.txt";
  std::string file_from2 = bucket_name + "/test_conditional_rename_from2.txt";
  std::string file_to = bucket_name + "/test_conditional_rename_to.txt";

  // Ensure source file does not exist
  fs_->DeleteFile(file_from);
  fs_->DeleteFile(file_from2);
  fs_->DeleteFile(file_to);

  std::string content1 = "This is a test file for conditional rename.";
  std::string content2 = "This is a test file for conditional rename 2.";
