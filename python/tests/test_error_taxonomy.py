...
@pytest.mark.parametrize(
    ...
    [
        (109, ErrorCategory.RETRYABLE),
        (102, ErrorCategory.CONFLICT),
        (117, ErrorCategory.DATA_FORMAT),
    ],
)
def test_native_categories_are_attributes_on_ffi_error(fake_lib, code, expected_category):
...
def test_error_category_matches_the_native_abi():
    ...
    native = {
        ...
        for name, value in re.findall(
            r"#define\s+LOON_ERROR_CATEGORY_([A-Z_]+)\s+(\d+)", header.read_text()
        )
    }
