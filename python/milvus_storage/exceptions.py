...
``category`` (the coarse handling hint). Nothing a program must decide on lives
in the message string: see ``docs/error-handling-rules.md`` R4.1-R4.3.
"""
...
class FFIError(MilvusStorageError):
    ...
    ``err_code``, ``category`` and ``retryable`` carry the structured verdict;
    callers do not need a parallel exception hierarchy for the same data.
