...
today. Python exposes `err_code`, `category`, and `retryable` as attributes on
one `FFIError`; caller-owned input errors use `InvalidArgumentError`. Milvus's Go
...
`ToSegcoreError(status)`: Retryable maps to a segcore code milvus itself treats
as retriable — `StorageTransientError` (2045) for every storage cause, and
`MemAllocateFailed` (2034) for allocation failure, which keeps triage pointed at
the node that ran out of memory instead of at the object store. Conflict and
System map to ordinary non-retriable storage codes. This mapping does not permit
reusing a failed writer. Conflict-aware handling uses the original
...
| `LOON_MEMORY_ERROR` | 2 | **Retryable** | **yes** | — | Allocation failure: the FFI surface's OOM code, mapping to segcore `2034 MemAllocateFailed` (never a storage or caller-fault code) |
...
| `StorageNoSuchUpload` | 101 | **Retryable** | **yes** | `NoSuchUpload` | 2045 `StorageTransientError` | Multipart upload state is gone; retry recreates the writer and upload |
...
Five values are missing from the tables above. None is available for reuse.
...
| 54 | Retired — was `PackedArrowError` (unclassified arrow failure, landed as `StorageError`); lower-layer classifications are preserved instead of being collapsed into one bucket |
...
The retired values are deliberately absent from the X-macro table, so they have
no name and no category: `loon_ffi_error_category` answers `Unknown` for them,
and they land in the conservative default branch. Value 54 still exports a
non-AWS tombstone symbol. The old `aws_*` aliases, including value 106,
were removed at the pre-release ABI break; their numeric values remain reserved
...
|---|---|---|---|---|
| 1 | `StorageNoSuchUpload` (101) | 404, client error | **Retryable** | Replaying against the dead upload id cannot help; an operation-aware retry discards the failed writer and starts a new upload. |
...
them. A dash in this documentation means there is no object-storage counterpart.
...
Repository-owned providers for Aliyun, Tencent, and Huawei return a typed
`ResolveForRequest()` result. The code is a function of the HTTP outcome, not
of which cloud it was:
...
| 429, bandwidth limit, or a provider throttling code such as AWS STS `ThrottlingException` on HTTP 400 | `StorageTransientThrottling` (109) |
...
Those providers make a bounded attempt -- `kCredentialRetryAttempts` retries
over the same request, with the body rewound between attempts -- and then
report. Their typed resolver is used both by construction preflight and before
an object request obtains a client. Deciding whether to try the whole operation
again later belongs to the caller, which knows what the operation was worth.

**AWS is excluded, on purpose.** Default, profile, AssumeRole, and WebIdentity
use the AWS SDK providers directly so Storage does not reimplement credential
precedence or silently change identity. Their public interface returns only
credentials, not a typed failure cause. If it returns an empty set, the SDK's
client-signing failure becomes an unclassified `IOError`: System and
non-retryable. Actual S3 responses still use the normal object-store
...
| Credential providers — Aliyun (RAM / OIDC chain / WebIdentity), Tencent, Huawei, GCP | **Yes** — 105/107/108/109/110/115 per the table above. **AWS is the exception:** SDK providers preserve identity precedence but expose no typed failure cause |
...
| Paimon (Rust `FileIO` / opendal, a separate IO stack that never reaches the C++ classifier) | **Partial** — the bridge tags five conditions (not-found, unusable config, not-implemented, throttling, service) through the universal transport marker; the former private text markers were retired (R3.1 done). Everything else opendal raises inside paimon -- its own 5xx, throttling and transport failures -- never reaches the classifier and stays unclassified |
