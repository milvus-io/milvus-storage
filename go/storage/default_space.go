...
func (s *DefaultSpace) Write(reader array.RecordReader, option *options.WriteOptions) error {
	...
	var (
		...
		scalarCols []arrow.Array
		vectorCols []arrow.Array
	...
	for reader.Next() {
		...
		for i := 0; i < int(rec.NumCols()); i++ {
			...
			if vectorSchema.HasField(rec.ColumnName(i)) {
				vectorCols = append(vectorCols, rec.Column(i))
			}
		}
		...
		if scalarWriter == nil {
			...
			_, err := parquet.NewFileWriter(scalarSchema, s.fs, scalarFilePath)
			if err != nil {
				return err
			}

			vectorFilePath := utils.GetNewParquetFilePath(s.manifest.SpaceOptions().Uri)
			_, err = parquet.NewFileWriter(vectorSchema, s.fs, vectorFilePath)
			if err != nil {
				return err
			}

			scalarFragment.AddFile(scalarFilePath)
			vectorFragment.AddFile(vectorFilePath)

		}

		scalarWriter.Write(scalarRecord)
		vectorWriter.Write(vectorRecord)

		if scalarWriter.Count() >= option.MaxRecordPerFile {
			scalarWriter.Close()
			vectorWriter.Close()
			scalarWriter = nil
			vectorWriter = nil
	...
	copiedManifest := s.manifest.Copy()
...
func (s *DefaultSpace) Write1(reader array.RecordReader, options *options.WriteOptions) error {
	// check schema consistency
	if !s.schema.Schema().Equal(reader.Schema()) {
		return ErrSchemaNotMatch
	}
